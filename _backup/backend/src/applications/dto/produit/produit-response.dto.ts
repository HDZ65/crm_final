export class ProduitDto {
  ...
  constructor(partial: Partial<ProduitDto>) {
    ...
    if (this.prix != null && this.tauxTVA != null) {
      this.prixTTC =
        Math.round(this.prix * (1 + this.tauxTVA / 100) * 100) / 100;

      // Calcul du prix promo si promotion active
      if (this.promotionActive && this.promotionPourcentage != null) {
        this.prixPromo =
          Math.round(this.prix * (1 - this.promotionPourcentage / 100) * 100) /
          100;
        this.prixPromoTTC =
          Math.round(this.prixPromo * (1 + this.tauxTVA / 100) * 100) / 100;
      }
    }
  }
}
