...
export class GetClientBaseUseCase {
  ...
  async executeWithContrats(id: string): Promise<ClientBaseWithContrats> {
    const entity = await this.repository.findByIdWithContrats(id);
  ...
  async executeAllWithContrats(
    filters?: ClientBaseFilters,
  ): Promise<ClientBaseWithContrats[]> {
    return await this.repository.findAllWithContrats(filters);
  }
}
