...
import {
  ...
  NiveauAlerte,
  TypeAlerte,
} from '../../dto/dashboard/alertes.dto';
...
export class GetAlertesUseCase {
  ...
  async execute(filters: DashboardFiltersDto): Promise<AlertesResponseDto> {
    ...
    const contratsEnAttente = await this.countContratsEnAttenteValidation(
      dateDebut,
      dateFin,
      organisationId,
    );
    ...
    return {
      ...
      nombreAvertissements: alertes.filter((a) => a.niveau === 'avertissement')
        .length,
  ...
  private async calculateTauxChurn(
    ...
    const totalContrats = await this.contratRepository
      .createQueryBuilder('contrat')
      .where('contrat.societeId = :societeId', { societeId })
      .andWhere('contrat.dateDebut <= :dateFin', {
        dateFin: dateFin.toISOString(),
      })
      .andWhere('(contrat.dateFin >= :dateDebut OR contrat.dateFin IS NULL)', {
        dateDebut: dateDebut.toISOString(),
      })
      .getCount();
  ...
  private async countContratsEnAttenteValidation(
    ...
    queryBuilder.where('contrat.createdAt <= :fivesDaysAgo', {
      fivesDaysAgo: fivesDaysAgo.toISOString(),
    });
    // Note: Le champ statut est une chaîne directe, pas une FK
    queryBuilder.andWhere('LOWER(contrat.statut) LIKE :statutAttente', {
