...
import {
  DashboardKpisDto,
  VariationDto,
} from '../../dto/dashboard/dashboard-kpis.dto';
...
export class GetDashboardKpisUseCase {
  ...
  async execute(filters: DashboardFiltersDto): Promise<DashboardKpisDto> {
    ...
    const contratsActifsPrecedent = await this.countContratsActifs(
      datePrecedenteDebut,
      datePrecedenteFin,
      organisationId,
      societeId,
    ...
    const mrrPrecedent = await this.calculateMRR(
      datePrecedenteDebut,
      datePrecedenteFin,
      organisationId,
      societeId,
    ...
    const tauxChurnPrecedent = await this.calculateTauxChurn(
      datePrecedenteDebut,
      datePrecedenteFin,
      organisationId,
      societeId,
    ...
    const tauxImpayesPrecedent = await this.calculateTauxImpayes(
      datePrecedenteDebut,
      datePrecedenteFin,
      organisationId,
      societeId,
    ...
    return {
      contratsActifs,
      contratsActifsVariation: this.calculateVariation(
        contratsActifs,
        contratsActifsPrecedent,
      ),
      mrr,
      mrrVariation: this.calculateVariation(mrr, mrrPrecedent),
      tauxChurn,
      tauxChurnVariation: this.calculateVariation(
        tauxChurn,
        tauxChurnPrecedent,
      ),
      tauxImpayes,
      tauxImpayesVariation: this.calculateVariation(
        tauxImpayes,
        tauxImpayesPrecedent,
      ),
    };
  }
  ...
  private async countContratsActifs(
    ...
    queryBuilder.where('contrat.dateDebut <= :dateFin', {
      dateFin: dateFin.toISOString(),
    });
    queryBuilder.andWhere(
      '(contrat.dateFin >= :dateDebut OR contrat.dateFin IS NULL)',
  ...
  private async calculateMRR(
    ...
    const result = await queryBuilder.getRawOne<{ total: string | null }>();
  ...
  private async calculateTauxImpayes(
    ...
    const totalQueryBuilder =
      this.factureRepository.createQueryBuilder('facture');
    totalQueryBuilder.select('SUM(facture.montantTTC)', 'total');
    totalQueryBuilder.where(
      'facture.dateEmission BETWEEN :dateDebut AND :dateFin',
      {
        dateDebut: dateDebut.toISOString(),
        dateFin: dateFin.toISOString(),
      },
    ...
    if (organisationId) {
      totalQueryBuilder.andWhere('facture.organisationId = :organisationId', {
        organisationId,
      });
    }
    ...
    if (organisationId) {
      impayesQueryBuilder.andWhere('facture.organisationId = :organisationId', {
        organisationId,
      });
    }
