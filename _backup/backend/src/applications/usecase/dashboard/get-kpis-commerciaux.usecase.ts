...
export class GetKpisCommerciauxUseCase {
  ...
  async execute(
    ...
    const { dateDebut, dateFin, organisationId, societeId } =
      this.resolveDates(filters);
    ...
    // 1. Nouveaux clients
    const nouveauxClientsMois = await this.countNouveauxClients(
      dateDebut,
      dateFin,
      organisationId,
    );
    ...
    const tauxConversion = await this.calculateTauxConversion(
      dateDebut,
      dateFin,
      organisationId,
      societeId,
    );
    const tauxConversionPrecedent = await this.calculateTauxConversion(
      datePrecedenteDebut,
      datePrecedenteFin,
      organisationId,
      societeId,
    ...
    // 3. Panier moyen
    const panierMoyen = await this.calculatePanierMoyen(
      dateDebut,
      dateFin,
      organisationId,
    );
    const panierMoyenPrecedent = await this.calculatePanierMoyen(
      datePrecedenteDebut,
      datePrecedenteFin,
      organisationId,
    );
    ...
    const caPrevisionnel3Mois = await this.calculateCAPrevisionnel(
      organisationId,
      societeId,
    );

    // 5. Classements commerciaux
    const classementParVentes = await this.getClassementParVentes(
      dateDebut,
      dateFin,
      organisationId,
      societeId,
    );
    const classementParCA = await this.getClassementParCA(
      dateDebut,
      dateFin,
      organisationId,
      societeId,
    );
    const classementParConversion = await this.getClassementParConversion(
      dateDebut,
      dateFin,
      organisationId,
      societeId,
    ...
    return {
      nouveauxClientsMois,
      nouveauxClientsVariation: this.calculateVariation(
        nouveauxClientsMois,
        nouveauxClientsPrecedent,
      ),
      tauxConversion,
      tauxConversionVariation: this.calculateVariation(
        tauxConversion,
        tauxConversionPrecedent,
      ),
      panierMoyen,
      panierMoyenVariation: this.calculateVariation(
        panierMoyen,
        panierMoyenPrecedent,
      ),
      caPrevisionnel3Mois,
      classementParVentes,
      classementParCA,
  ...
  private async calculateTauxConversion(
    ...
    const contratsSignesQuery =
      this.contratRepository.createQueryBuilder('contrat');
    contratsSignesQuery.where(
      'contrat.dateSignature BETWEEN :dateDebut AND :dateFin',
      {
    ...
    if (organisationId) {
      contratsSignesQuery.andWhere('contrat.organisationId = :organisationId', {
        organisationId,
      });
    }
    if (societeId) {
      contratsSignesQuery.andWhere('contrat.societeId = :societeId', {
        societeId,
      });
    }
    ...
    if (organisationId) {
      totalClientsQuery.andWhere('client.organisationId = :organisationId', {
        organisationId,
      });
    }
  ...
  private async calculatePanierMoyen(
    ...
    if (organisationId) {
      queryBuilder.andWhere('facture.organisationId = :organisationId', {
        organisationId,
      });
    }
  ...
  private async calculateCAPrevisionnel(
    ...
    queryBuilder.select(
      'SUM(ligne.quantite * ligne.prixUnitaire * 3)',
      'total',
    );

    // Contrats actifs (en cours)
    queryBuilder.where('contrat.dateDebut <= :now', {
      now: now.toISOString().split('T')[0],
    });
    queryBuilder.andWhere(
      '(contrat.dateFin >= :now OR contrat.dateFin IS NULL)',
      { now: now.toISOString().split('T')[0] },
    );
  ...
  private async getClassementParVentes(
    ...
    queryBuilder.addSelect(
      "CONCAT(commercial.prenom, ' ', commercial.nom)",
      'nomcomplet',
    );
  ...
  private async getClassementParConversion(
    ...
    queryBuilder.addSelect(
      'COUNT(DISTINCT contrat.clientId)',
      'clients_convertis',
    );
    queryBuilder.where(
      'contrat.dateSignature BETWEEN :dateDebut AND :dateFin',
    ...
    for (const r of results) {
      ...
      const tauxConversion =
        totalContrats > 0
          ? parseFloat(
              (
                (parseInt(r.clients_convertis, 10) / totalContrats) *
                100
              ).toFixed(1),
            )
          : 0;
  ...
  private calculateVariation(
    ...
    return {
      ...
      tendance:
        pourcentage > 0 ? 'hausse' : pourcentage < 0 ? 'baisse' : 'stable',
    };
  }
}
