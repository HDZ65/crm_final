...
export class UpdateRegleRelanceUseCase {
  ...
  async execute(
    ...
    if (dto.declencheur !== undefined)
      updateData.declencheur = dto.declencheur as RelanceDeclencheur;
    if (dto.delaiJours !== undefined) updateData.delaiJours = dto.delaiJours;
    if (dto.actionType !== undefined)
      updateData.actionType = dto.actionType as RelanceActionType;
    if (dto.prioriteTache !== undefined)
      updateData.prioriteTache = dto.prioriteTache as
        | 'HAUTE'
        | 'MOYENNE'
        | 'BASSE';
    if (dto.templateEmailId !== undefined)
      updateData.templateEmailId = dto.templateEmailId;
    if (dto.templateTitreTache !== undefined)
      updateData.templateTitreTache = dto.templateTitreTache;
    if (dto.templateDescriptionTache !== undefined)
      updateData.templateDescriptionTache = dto.templateDescriptionTache;
    if (dto.assigneParDefaut !== undefined)
      updateData.assigneParDefaut = dto.assigneParDefaut;
