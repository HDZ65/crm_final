...
export class UpdateTacheUseCase {
  ...
  async execute(id: string, dto: UpdateTacheDto): Promise<TacheEntity> {
    ...
    if (dto.priorite !== undefined)
      updateData.priorite = dto.priorite as TachePriorite;
    if (dto.statut !== undefined) updateData.statut = dto.statut as TacheStatut;
    if (dto.dateEcheance !== undefined)
      updateData.dateEcheance = new Date(dto.dateEcheance);
    if (dto.dateCompletion !== undefined)
      updateData.dateCompletion = new Date(dto.dateCompletion);
