...
class CreateInvitationDto {
  ...
  @ApiProperty({
    description: 'ID du rôle (optionnel, défaut: member)',
    required: false,
  })
  @IsOptional()
  @IsUUID()
...
export class InvitationCompteController {
  ...
  @ApiOperation({
    ...
    description:
      "Crée une invitation pour qu'un utilisateur rejoigne l'organisation",
  })
  @ApiResponse({ status: 201, description: 'Invitation créée avec succès' })
  @ApiResponse({ status: 401, description: 'Non autorisé' })
  @ApiResponse({
    status: 403,
    description: "Pas propriétaire de l'organisation",
  })
  ...
  async createInvitation(
    ...
    const alreadyMember = members.find(
      (m) => m.utilisateur?.email === dto.emailInvite,
    );
    if (alreadyMember) {
      throw new BadRequestException(
    ...
    this.logger.log(
      `Invitation créée: ${saved.id} pour ${dto.emailInvite} dans ${organisation.nom}`,
    );
  ...
  @ApiOperation({
    ...
    description:
      "Vérifie si le token d'invitation est valide et retourne les informations",
  })
  ...
  async validateInvitation(
    ...
    if (invitation.etat !== 'pending') {
      throw new BadRequestException(
        `Cette invitation a déjà été ${invitation.etat === 'accepted' ? 'acceptée' : 'annulée'}`,
      );
    }
  ...
  @ApiOperation({
    ...
    description:
      "Accepte l'invitation et associe l'utilisateur à l'organisation",
  })
  ...
  async acceptInvitation(
    ...
    this.logger.log(
      `Utilisateur synchronisé pour acceptation: ${dbUser.email} (id: ${dbUser.id})`,
    );
    ...
    this.logger.log(
      `Membre créé: ${savedMembre.id} pour organisation ${invitation.organisationId}`,
    );
    ...
    this.logger.log(
      `Notifications envoyées aux ${existingMembers.length - 1} membre(s) de l'organisation`,
    );
