...
export class McpService {
  ...
  async execute(name: string, input: unknown, ctx: { requestId: string }) {
    this.logger.log(
      `[${ctx.requestId}] tool=${name} input=${JSON.stringify(input).slice(0, 500)}`,
    );
  ...
  private async searchClients(input: SearchClientsInput) {
    ...
    const scoped = entities.filter(
      (e: any) => !e.organisationId || e.organisationId === organisationId,
    );
  ...
  private async getClient(input: GetClientInput) {
    ...
    if (
      (entity as any).organisationId &&
      (entity as any).organisationId !== organisationId
    ) {
  ...
  private async updateActivity(input: UpdateActivityInput) {
    ...
    if (
      ...
      (existing as any).organisationId !== organisationId
    ) {
      throw new Error('Forbidden: wrong tenant');
    }
