...
import type {
  RegleRelanceEntity as RegleRelanceDomainEntity,
  RelanceDeclencheur,
...
export class TypeOrmRegleRelanceRepository implements RegleRelanceRepositoryPort {
  ...
  ): Promise<RegleRelanceDomainEntity[]> {
    const entities = await this.repository.find({
  ...
  async findActivesByDeclencheur(
    organisationId: string,
    declencheur: RelanceDeclencheur,
  ): Promise<RegleRelanceDomainEntity[]> {
    const entities = await this.repository.find({
  ...
  async create(
    entity: RegleRelanceDomainEntity,
  ): Promise<RegleRelanceDomainEntity> {
    const ormEntity = this.repository.create(
      RegleRelanceMapper.toPersistence(entity),
    );
  ...
  async update(
    id: string,
    entity: Partial<RegleRelanceDomainEntity>,
  ): Promise<RegleRelanceDomainEntity> {
    await this.repository.update(
      id,
      RegleRelanceMapper.toPersistence(entity as RegleRelanceDomainEntity),
    );
