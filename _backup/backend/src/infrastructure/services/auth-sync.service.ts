...
export class AuthSyncService {
  ...
  async syncKeycloakUser(
    ...
    let utilisateur =
      await this.utilisateurRepository.findByKeycloakId(keycloakId);

    if (utilisateur) {
      this.logger.log(
        `Utilisateur existant trouvé: ${utilisateur.email} (keycloakId: ${keycloakId})`,
      );
      return utilisateur;
    }
    ...
    this.logger.log(
      `Création d'un nouvel utilisateur pour keycloakId: ${keycloakId}`,
    );
    ...
    this.logger.log(
      `Nouvel utilisateur créé: ${utilisateur.email} (id: ${utilisateur.id})`,
    );
  ...
  async findByKeycloakId(
    keycloakId: string,
  ): Promise<UtilisateurEntity | null> {
