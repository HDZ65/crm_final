...
export class RelanceEngineService {
  ...
  async executeRelances(): Promise<void> {
    ...
    try {
      ...
      const organisationIds = [
        ...new Set(
          allRegles.filter((r) => r.actif).map((r) => r.organisationId),
        ),
      ];
      ...
      this.logger.error(
        "Erreur lors de l'exécution du moteur de relances",
        error,
      );
    }
  }
  ...
  async executeRelancesForOrganisation(organisationId: string): Promise<void> {
    this.logger.debug(
      `Traitement des relances pour l'organisation ${organisationId}`,
    );

    const reglesActives =
      await this.regleRelanceRepository.findActives(organisationId);
    ...
    for (const regle of reglesActives) {
      try {
        ...
        this.logger.error(
          `Erreur lors de l'exécution de la règle ${regle.id}`,
          error,
        );
      }
    }
  ...
  private async executeRegle(regle: RegleRelanceEntity): Promise<void> {
    this.logger.debug(
      `Exécution de la règle: ${regle.nom} (${regle.declencheur})`,
    );
  ...
  private async processContratBientotExpire(
  ...
  private async createRelanceIfNeeded(
    ...
    if (alreadyExists) {
      this.logger.debug(
        `Relance déjà effectuée aujourd'hui pour la règle ${regle.id}`,
      );
      return;
    }
    ...
    try {
      ...
      if (
        regle.actionType === 'CREER_TACHE' ||
        regle.actionType === 'TACHE_ET_EMAIL'
      ) {
  ...
  private async createNotificationFromRegle(
    ...
    const message = this.formatTemplate(
      regle.templateDescriptionTache || this.getDefaultDescription(regle),
      context,
    );
  ...
  private getNotificationTypeFromDeclencheur(
