...
import { RepartitionProduitsResponseDto, RepartitionProduitDto } from '../../dto/dashboard/repartition-produits.dto';
...
export class GetRepartitionProduitsUseCase {
  ...
  async execute(filters: DashboardFiltersDto): Promise<RepartitionProduitsResponseDto> {
    const { organisationId } = filters;

    // Récupérer la répartition du CA par produit via les lignes de contrat
    const queryBuilder = this.ligneContratRepository.createQueryBuilder('ligne');
    ...
    const caTotal = results.reduce((sum, r) => sum + parseFloat(r.ca || '0'), 0);
    ...
    const produitsResponse: RepartitionProduitDto[] = results.map((r, index) => {
      const ca = parseFloat(r.ca || '0');
      return {
        produitId: r.produitId,
        nomProduit: produitsMap.get(r.produitId) || 'Produit inconnu',
        ca,
        pourcentage: caTotal > 0 ? parseFloat(((ca / caTotal) * 100).toFixed(1)) : 0,
