...
export class GetStatsSocietesUseCase {
  ...
  async execute(filters: DashboardFiltersDto): Promise<StatsSocietesResponseDto> {
    ...
    const statsPromises = societes.map(async (societe) => {
      const stats = await this.calculateStatsForSociete(societe.id, dateDebut, dateFin, organisationId);
      return {
  ...
  private async calculateStatsForSociete(
    ...
    // Contrats actifs
    const contratsActifs = await this.countContratsActifs(societeId, dateDebut, dateFin);
    ...
    // Nouveaux clients
    const nouveauxClients = await this.countNouveauxClients(societeId, dateDebut, dateFin);
    const nouveauxClientsPrecedent = await this.countNouveauxClients(societeId, datePrecedenteDebut, datePrecedenteFin);
    const nouveauxClientsVariation = nouveauxClientsPrecedent > 0
      ? parseFloat((((nouveauxClients - nouveauxClientsPrecedent) / nouveauxClientsPrecedent) * 100).toFixed(0))
      : (nouveauxClients > 0 ? 100 : 0);

    // Taux de churn
    const tauxChurn = await this.calculateTauxChurn(societeId, dateDebut, dateFin);

    // Taux d'impayés
    const tauxImpayes = await this.calculateTauxImpayes(societeId, dateDebut, dateFin);
  ...
  private async calculateMRR(societeId: string, dateDebut: Date, dateFin: Date): Promise<number> {
    const result = await this.factureRepository.createQueryBuilder('facture')
  ...
  private async countNouveauxClients(societeId: string, dateDebut: Date, dateFin: Date): Promise<number> {
    // Compter les clients qui ont leur premier contrat dans la période
    const result = await this.contratRepository.createQueryBuilder('contrat')
      .select('COUNT(DISTINCT contrat.clientBaseId)', 'count')
      .where('contrat.societeId = :societeId', { societeId })
  ...
  private async calculateTauxChurn(societeId: string, dateDebut: Date, dateFin: Date): Promise<number> {
    ...
    const totalContrats = await this.countContratsActifs(societeId, dateDebut, dateFin);
