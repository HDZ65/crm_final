...
export class GetNotificationUseCase {
  ...
  async executeByUtilisateurId(utilisateurId: string): Promise<NotificationEntity[]> {
    return await this.repository.findByUtilisateurId(utilisateurId);
  }

  async executeUnreadByUtilisateurId(utilisateurId: string): Promise<NotificationEntity[]> {
