...
export class GetRegleRelanceUseCase {
  ...
  async executeByOrganisationId(organisationId: string): Promise<RegleRelanceEntity[]> {
  ...
  async executeByDeclencheur(organisationId: string, declencheur: RelanceDeclencheur): Promise<RegleRelanceEntity[]> {
    return await this.repository.findByDeclencheur(organisationId, declencheur);
  }

  async executeActivesByDeclencheur(organisationId: string, declencheur: RelanceDeclencheur): Promise<RegleRelanceEntity[]> {
    return await this.repository.findActivesByDeclencheur(organisationId, declencheur);
  }
}
