...
import { TacheEntity, TacheStatut, TacheType } from '../../../core/domain/tache.entity';
...
export class GetTacheUseCase {
  ...
  async executeByOrganisationId(organisationId: string): Promise<TacheEntity[]> {
  ...
  async executeByStatut(organisationId: string, statut: TacheStatut): Promise<TacheEntity[]> {
    return await this.repository.findByStatut(organisationId, statut);
  }

  async executeByType(organisationId: string, type: TacheType): Promise<TacheEntity[]> {
  ...
  async executeCountByStatut(organisationId: string): Promise<Record<TacheStatut, number>> {
