...
export class DashboardController {
  ...
  @ApiResponse({ status: 200, description: 'KPIs récupérés avec succès', type: DashboardKpisDto })
  ...
  @ApiQuery({ name: 'produitId', required: false, description: 'Filtrer par produit' })
  @ApiQuery({ name: 'canal', required: false, description: 'Filtrer par canal' })
  ...
  @ApiQuery({
    name: 'periodeRapide',
    required: false,
    enum: ['mois_courant', 'mois_dernier', 'trimestre_courant', 'annee_courante', 'personnalisee'],
    description: 'Période rapide prédéfinie'
  })
  async getKpis(@Query() filters: DashboardFiltersDto): Promise<DashboardKpisDto> {
    return this.getDashboardKpisUseCase.execute(filters);
  }

  @Get('evolution-ca')
  @ApiOperation({ summary: 'Récupère l\'évolution du CA sur 12 mois' })
  @ApiResponse({ status: 200, description: 'Evolution CA récupérée avec succès', type: EvolutionCaResponseDto })
  @ApiQuery({ name: 'organisationId', required: false, description: 'Filtrer par organisation' })
  @ApiQuery({ name: 'societeId', required: false, description: 'Filtrer par société' })
  @ApiQuery({ name: 'dateDebut', required: false, description: 'Date de début (YYYY-MM-DD)' })
  @ApiQuery({ name: 'dateFin', required: false, description: 'Date de fin (YYYY-MM-DD)' })
  ...
  async getEvolutionCa(@Query() filters: DashboardFiltersDto): Promise<EvolutionCaResponseDto> {
  ...
  @ApiResponse({ status: 200, description: 'Répartition produits récupérée avec succès', type: RepartitionProduitsResponseDto })
  @ApiQuery({ name: 'organisationId', required: false, description: 'Filtrer par organisation' })
  @ApiQuery({ name: 'societeId', required: false, description: 'Filtrer par société' })
  async getRepartitionProduits(@Query() filters: DashboardFiltersDto): Promise<RepartitionProduitsResponseDto> {
    return this.getRepartitionProduitsUseCase.execute(filters);
  }

  @Get('stats-societes')
  @ApiOperation({ summary: 'Récupère les statistiques par société' })
  @ApiResponse({ status: 200, description: 'Stats sociétés récupérées avec succès', type: StatsSocietesResponseDto })
  @ApiQuery({ name: 'organisationId', required: false, description: 'Filtrer par organisation' })
  @ApiQuery({ name: 'dateDebut', required: false, description: 'Date de début (YYYY-MM-DD)' })
  @ApiQuery({ name: 'dateFin', required: false, description: 'Date de fin (YYYY-MM-DD)' })
  @ApiQuery({
  ...
  async getStatsSocietes(@Query() filters: DashboardFiltersDto): Promise<StatsSocietesResponseDto> {
    return this.getStatsSocietesUseCase.execute(filters);
  }

  @Get('alertes')
  @ApiOperation({ summary: 'Récupère les alertes et risques actifs' })
  @ApiResponse({ status: 200, description: 'Alertes récupérées avec succès', type: AlertesResponseDto })
  @ApiQuery({ name: 'organisationId', required: false, description: 'Filtrer par organisation' })
  @ApiQuery({ name: 'dateDebut', required: false, description: 'Date de début (YYYY-MM-DD)' })
  @ApiQuery({ name: 'dateFin', required: false, description: 'Date de fin (YYYY-MM-DD)' })
  @ApiQuery({
  ...
  async getAlertes(@Query() filters: DashboardFiltersDto): Promise<AlertesResponseDto> {
    return this.getAlertesUseCase.execute(filters);
  }

  @Get('kpis-commerciaux')
  @ApiOperation({ summary: 'Récupère les KPIs commerciaux (nouveaux clients, taux conversion, panier moyen, classements)' })
  @ApiResponse({ status: 200, description: 'KPIs commerciaux récupérés avec succès', type: KpisCommerciauxResponseDto })
  @ApiQuery({ name: 'organisationId', required: false, description: 'Filtrer par organisation' })
  @ApiQuery({ name: 'societeId', required: false, description: 'Filtrer par société' })
  @ApiQuery({ name: 'dateDebut', required: false, description: 'Date de début (YYYY-MM-DD)' })
  @ApiQuery({ name: 'dateFin', required: false, description: 'Date de fin (YYYY-MM-DD)' })
  ...
  async getKpisCommerciaux(@Query() filters: DashboardFiltersDto): Promise<KpisCommerciauxResponseDto> {
