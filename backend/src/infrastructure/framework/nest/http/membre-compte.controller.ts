...
export class MembreCompteController {
  ...
  async leaveOrganisation(
    ...
    const role = await this.roleRepository.findOne({ where: { id: membre.roleId } });
    if (role?.code === 'owner') {
      throw new ForbiddenException('Le propriétaire ne peut pas quitter l\'organisation. Transférez la propriété ou supprimez l\'organisation.');
    }
    ...
    if (ownerRole && organisation) {
      ...
      for (const owner of owners) {
        ...
        this.notificationGateway.notifyNewNotification(owner.utilisateurId, savedNotification);
      }
    }
  }
  ...
  async removeMember(
    ...
    const currentRole = await this.roleRepository.findOne({ where: { id: currentMembre.roleId } });
    if (currentRole?.code !== 'owner') {
      throw new ForbiddenException('Seul le propriétaire peut supprimer des membres');
    }
    ...
    const memberRole = await this.roleRepository.findOne({ where: { id: membreToRemove.roleId } });
    ...
    if (organisation && membreToRemove.utilisateur) {
      ...
      const userName = [removedUser.prenom, removedUser.nom].filter(Boolean).join(' ') || removedUser.email;
      ...
      const savedNotifRemoved = await this.notificationRepository.save(notificationForRemoved);
      this.notificationGateway.notifyNewNotification(removedUser.id, savedNotifRemoved);
  ...
  async updateMemberRole(
    ...
    const newRole = await this.roleRepository.findOne({ where: { id: body.roleId } });
    if (!newRole) {
      throw new NotFoundException('Rôle non trouvé');
    }

    // 6. Empêcher de changer le rôle d'un owner
    const memberCurrentRole = await this.roleRepository.findOne({ where: { id: membreToUpdate.roleId } });
    ...
    if (newRole.code === 'owner') {
      throw new ForbiddenException('Impossible de promouvoir en propriétaire via cette action');
    }
