...
export class TypeOrmHistoriqueRelanceRepository implements HistoriqueRelanceRepositoryPort {
  ...
  async findByOrganisationId(organisationId: string): Promise<HistoriqueRelanceDomainEntity[]> {
    const entities = await this.repository.find({
  ...
  async findByRegleRelanceId(regleRelanceId: string): Promise<HistoriqueRelanceDomainEntity[]> {
    const entities = await this.repository.find({
  ...
  async findByClientId(clientId: string): Promise<HistoriqueRelanceDomainEntity[]> {
    const entities = await this.repository.find({
      where: { clientId },
      order: { dateExecution: 'DESC' },
    });
  ...
  async findByContratId(contratId: string): Promise<HistoriqueRelanceDomainEntity[]> {
    const entities = await this.repository.find({
      where: { contratId },
      order: { dateExecution: 'DESC' },
    });
  ...
  async findByFactureId(factureId: string): Promise<HistoriqueRelanceDomainEntity[]> {
    const entities = await this.repository.find({
      where: { factureId },
      order: { dateExecution: 'DESC' },
    });
  ...
  async findRecent(organisationId: string, limit: number): Promise<HistoriqueRelanceDomainEntity[]> {
    const entities = await this.repository.find({
      where: { organisationId },
      order: { dateExecution: 'DESC' },
      take: limit,
  ...
  async create(entity: HistoriqueRelanceDomainEntity): Promise<HistoriqueRelanceDomainEntity> {
    const ormEntity = this.repository.create(HistoriqueRelanceMapper.toPersistence(entity));
    const saved = await this.repository.save(ormEntity);
    return HistoriqueRelanceMapper.toDomain(saved);
  }

  async update(id: string, entity: Partial<HistoriqueRelanceDomainEntity>): Promise<HistoriqueRelanceDomainEntity> {
    await this.repository.update(id, HistoriqueRelanceMapper.toPersistence(entity as HistoriqueRelanceDomainEntity));
