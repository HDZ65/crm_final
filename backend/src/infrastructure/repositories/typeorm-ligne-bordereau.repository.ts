...
@Injectable()
export class TypeOrmLigneBordereauRepository
  implements LigneBordereauRepositoryPort
{
  constructor(
  ...
  async update(
    ...
    await this.repository.update(id, LigneBordereauMapper.toPersistence(entity));
  ...
  async findByBordereauId(bordereauId: string): Promise<LigneBordereauEntity[]> {
    const ormEntities = await this.repository.find({
      where: { bordereauId },
      order: { ordre: 'ASC' },
    });
