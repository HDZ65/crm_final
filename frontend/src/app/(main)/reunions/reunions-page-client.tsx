...
interface ReunionsPageClientProps {
  initialOrgId?: string | null
}

export function ReunionsPageClient({ initialOrgId }: ReunionsPageClientProps) {
  const { user } = useAuth()
  ...
  const fetchAgendaData = React.useCallback(async () => {
    ...
    // TODO: backend agenda service not implemented yet — skip gRPC calls
    setCalendarEvents([])
    setMeetings([])
  ...
  const handleOpenEditEventDialog = React.useCallback((eventItem: CalendarEvent) => {
  ...
  const handleMatchParticipants = React.useCallback(async (meetingId: string) => {
    ...
    try {
      ...
      const matched = result.data?.matchedCount ?? 0
  ...
  const handleLoadSummary = React.useCallback(async (meetingId: string) => {
  ...
  const handleDeleteEventFromTable = React.useCallback((eventItem: CalendarEvent) => {
    setDeletingEvent(eventItem)
    setEventDeleteDialogOpen(true)
  }, [])

  const handleShowMeetingSummary = React.useCallback((meetingId: string) => {
    setActiveTab("summaries")
    void handleLoadSummary(meetingId)
  }, [handleLoadSummary])

  const handleMatchParticipantsFromTable = React.useCallback((meetingId: string) => {
    void handleMatchParticipants(meetingId)
  }, [handleMatchParticipants])

  const handleLoadSummaryFromTable = React.useCallback((meetingId: string) => {
    void handleLoadSummary(meetingId)
  }, [handleLoadSummary])
  ...
  const eventColumns = React.useMemo(
    () =>
      createEventColumns({
        onEdit: handleOpenEditEventDialog,
        onDelete: handleDeleteEventFromTable,
      }),
    [handleOpenEditEventDialog, handleDeleteEventFromTable]
  )

  const meetingColumns = React.useMemo(
    () =>
      createMeetingColumns({
        onMatchParticipants: handleMatchParticipantsFromTable,
        onShowSummary: handleShowMeetingSummary,
        matchingMeetingId,
      }),
    [handleMatchParticipantsFromTable, handleShowMeetingSummary, matchingMeetingId]
  )

  const summaryColumns = React.useMemo(
    () =>
      createSummaryColumns({
        onLoadSummary: handleLoadSummaryFromTable,
        selectedMeetingId,
      }),
    [handleLoadSummaryFromTable, selectedMeetingId]
  )
