...
interface ConfigurationRelancesPageClientProps {
  initialRegles?: RegleRelanceDto[]
  initialHistorique?: import("@/lib/ui/labels/regle-relance").HistoriqueRelanceDto[]
  activeOrgId?: string | null
}

export function ConfigurationRelancesPageClient({
  initialRegles,
  initialHistorique,
  activeOrgId,
}: ConfigurationRelancesPageClientProps) {
  const { activeOrganisation } = useOrganisation()

  // State for data from Server Actions
  const [regles, setRegles] = React.useState<RegleRelanceDto[]>(initialRegles ?? [])
  const [historique, setHistorique] = React.useState<import("@/lib/ui/labels/regle-relance").HistoriqueRelanceDto[]>(initialHistorique ?? [])
  const [loading, setLoading] = React.useState(!initialRegles)
