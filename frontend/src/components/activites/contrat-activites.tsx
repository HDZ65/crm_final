...
import { listActivitesByContrat } from "@/actions/activites"
...
interface ContratActivitesProps {
  contratId: string
}

export function ContratActivites({ contratId }: ContratActivitesProps) {
  ...
  const fetchData = React.useCallback(async () => {
    ...
    const [activitesResult, typesResult] = await Promise.all([
      listActivitesByContrat(contratId),
      listTypesActivite(),
    ])
