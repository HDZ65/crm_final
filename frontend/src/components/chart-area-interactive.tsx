...
import { Maximize2, TrendingUp, Download, Euro, AlertTriangle, RefreshCw, BarChart3 } from "lucide-react"
...
interface ChartAreaInteractiveProps {
  /** Initial evolution CA data from server - skips client-side fetch if provided */
  initialData?: EvolutionCaMensuelle[] | null
}
...
export function ChartAreaInteractive({ initialData }: ChartAreaInteractiveProps) {
  ...
  const [chartData, setChartData] = React.useState<EvolutionCaMensuelle[]>(initialData || [])
  ...
  const fetchData = React.useCallback(async () => {
    ...
    const result = await getEvolutionCa({
    ...
    if (result.error) {
      ...
      setChartData(result.data.donnees)
    }
    ...
    return (
      <Card className="h-full flex items-center justify-center flex-col gap-3 p-6">
        ...
        <div className="text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1">Les données de CA seront affichées ici</p>
        </div>
      </Card>
    )
  }
  ...
  if (!chartData || chartData.length === 0) {
    return (
      <Card className="h-full flex items-center justify-center flex-col gap-3 p-6">
        ...
        <div className="text-center">
          <p className="text-sm font-medium text-foreground">Prêt à démarrer</p>
          <p className="text-xs text-muted-foreground mt-1 max-w-xs">
            Les données d'évolution du CA apparaîtront ici dès que des contrats seront enregistrés.
          </p>
        </div>
      </Card>
    )
