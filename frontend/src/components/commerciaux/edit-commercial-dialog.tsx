...
import { useUpdateApporteur } from "@/hooks/commissions/use-commission-mutations"
...
export function EditCommercialDialog({ open, onOpenChange, commercial, onSuccess }: EditCommercialDialogProps) {
  const { update, loading } = useUpdateApporteur()
  ...
  const onSubmit = async (data: EditCommercialFormValues) => {
    const result = await update(commercial.id, {
      nom: data.nom,
      prenom: data.prenom,
      typeApporteur: data.typeApporteur as TypeApporteur,
