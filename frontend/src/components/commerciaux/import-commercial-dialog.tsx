...
import { useCreateApporteur } from "@/hooks/commissions/use-commission-mutations"
import type { TypeApporteur } from "@/types/commission-dto"
...
export function ImportCommercialDialog({ open, onOpenChange, onSuccess }: ImportCommercialDialogProps) {
  ...
  const { create, loading } = useCreateApporteur()
    ...
    for (let i = 0; i < preview.data.length; i++) {
      ...
      try {
        const response = await create({
          ...
          typeApporteur: commercial.typeApporteur,
        ...
        if (response) {
          ...
          importResult.errors.push({ line: i + 2, error: "Échec de création" })
        }
      } catch (err) {
        ...
        <DialogFooter>
          ...
          {!result && (
            <Button
              ...
              disabled={!preview || preview.data.length === 0 || importing || loading}
            >
              {importing ? (
                <>Importation...</>
              ) : (
