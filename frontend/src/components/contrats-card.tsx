...
interface ContratsCardProps {
  /** Initial stats societes data from server - skips client-side fetch if provided */
  initialData?: StatsSociete[] | null
}

export function ContratsCard({ initialData }: ContratsCardProps) {
  ...
  const fetchData = React.useCallback(async () => {
    ...
    const result = await getStatsSocietes({
    ...
    if (result.error) {
      ...
      setTableData(result.data.societes)
    }

    setLoading(false)
  }, [activeOrganisation, periodDates.dateDebut, periodDates.dateFin])

  // Only fetch client-side if no initial data was provided OR period changes
  React.useEffect(() => {
  ...
  // Refetch when period changes (user interaction)
  React.useEffect(() => {
    if (hasFetched.current && activeOrganisation) {
      fetchData()
    }
    ...
    return (
      <Card className="h-full flex items-center justify-center flex-col gap-3 p-6">
        ...
        <div className="text-center">
          <p className="text-sm font-medium text-foreground">Sélectionnez une organisation</p>
          <p className="text-xs text-muted-foreground mt-1">La répartition des contrats sera affichée ici</p>
        </div>
      </Card>
    )
  }
  ...
  if (error) {
    return (
      <Card className="h-full flex items-center justify-center flex-col gap-3 p-6">
        ...
        <Button variant="outline" size="sm" onClick={fetchData} className="gap-2 mt-2">
  ...
  // No data state with positive messaging
  if (!tableData || tableData.length === 0) {
    return (
      <Card className="h-full flex items-center justify-center flex-col gap-3 p-6">
        ...
        <div className="text-center">
          <p className="text-sm font-medium text-foreground">Aucun contrat pour le moment</p>
          <p className="text-xs text-muted-foreground mt-1 max-w-xs">
            Les contrats apparaîtront ici dès qu'ils seront créés dans le système.
          </p>
        </div>
      </Card>
    )
