...
import { useCreateOrganisation, useAuth } from "@/hooks/auth"
...
export function CreateOrganizationDialog({
    ...
    const onSubmit = async (data: OrganizationFormValues) => {
        ...
        try {
            const result = await createOrganisation(data.name, {
                sub: profile.id,
                email: profile.email,
            ...
            if (result?.compte) {
                ...
                onOpenChange(false)
                form.reset()
                onSuccess?.({ id: result.compte.id, nom: result.compte.nom })
            }
        } catch (error: unknown) {
