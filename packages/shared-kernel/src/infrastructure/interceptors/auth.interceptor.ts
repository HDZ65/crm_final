...
export class AuthInterceptor implements NestInterceptor {
  ...
  private extractAuthorization(metadata: { get?: (key: string) => unknown[] }): string | undefined {
  ...
  private isInternalCall(metadata: { get?: (key: string) => unknown[] }): boolean {
    if (!metadata || typeof metadata.get !== 'function') {
      return false;
    }
  ...
  private isPublicEndpoint(context: ExecutionContext): boolean {
    ...
    const publicMarkers = ['health', 'live', 'ready', 'readiness', 'liveness', 'ping'];
    ...
    return publicMarkers.some(
      (marker) => handlerName.includes(marker) || className.includes(marker) || pathValue.includes(marker),
    );
  }
}
