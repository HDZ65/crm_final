...
import { DateShiftStrategy } from '../configuration/entities/system-debit-configuration.entity.js';
let CsvImportService = CsvImportService_1 = class CsvImportService {
    ...
    dataSource;
    auditService;
    logger = new Logger(CsvImportService_1.name);
    constructor(companyConfigRepo, clientConfigRepo, contractConfigRepo, holidayRepo, dataSource, auditService) {
    ...
    async importCsv(organisationId, importType, csvContent, isDryRun, actorUserId) {
        ...
        switch (importType) {
            case 'company_config':
                return this.importCompanyConfigs(organisationId, records, isDryRun, actorUserId, result);
            case 'client_config':
                return this.importClientConfigs(organisationId, records, isDryRun, actorUserId, result);
            case 'contract_config':
                return this.importContractConfigs(organisationId, records, isDryRun, actorUserId, result);
            case 'holidays':
                return this.importHolidays(records, isDryRun, actorUserId, result);
            default:
                result.errors.push({ row: 0, message: `Unknown import type: ${importType}` });
                return result;
        }
    }
    async importCompanyConfigs(organisationId, rows, isDryRun, actorUserId, result) {
        const validatedRows = [];
        for (let i = 0; i < rows.length; i++) {
            const row = rows[i];
            const rowNum = i + 2;
            const validation = this.validateConfigRow(row, rowNum, ['societe_id']);
        ...
        try {
            for (const { data, existing } of validatedRows) {
                const configData = {
                    ...
                    mode: data.mode,
                    batch: data.batch ? data.batch : undefined,
                    fixedDay: data.fixed_day ? parseInt(data.fixed_day, 10) : undefined,
                    shiftStrategy: data.shift_strategy ?? DateShiftStrategy.NEXT_BUSINESS_DAY,
            ...
            await this.auditService.logBulkImport(organisationId, 'company_config', result.importId, actorUserId, `Imported ${result.createdCount} new, updated ${result.updatedCount} existing company configurations`, { totalRows: result.totalRows, validRows: result.validRows, errorRows: result.errorRows });
        }
        catch (error) {
    ...
    async importClientConfigs(organisationId, rows, isDryRun, actorUserId, result) {
        const validatedRows = [];
        for (let i = 0; i < rows.length; i++) {
            const row = rows[i];
            const rowNum = i + 2;
            const validation = this.validateConfigRow(row, rowNum, ['client_id']);
            if (validation.errors.length > 0) {
                result.errors.push(...validation.errors);
                result.errorRows++;
                continue;
        ...
        try {
            ...
            await queryRunner.commitTransaction();
            await this.auditService.logBulkImport(organisationId, 'client_config', result.importId, actorUserId, `Imported ${result.createdCount} new, updated ${result.updatedCount} existing client configurations`, { totalRows: result.totalRows, validRows: result.validRows, errorRows: result.errorRows });
        }
        catch (error) {
            await queryRunner.rollbackTransaction();
            throw error;
    ...
    async importContractConfigs(organisationId, rows, isDryRun, actorUserId, result) {
        const validatedRows = [];
        for (let i = 0; i < rows.length; i++) {
            const row = rows[i];
            const rowNum = i + 2;
            const validation = this.validateConfigRow(row, rowNum, ['contrat_id']);
            if (validation.errors.length > 0) {
                result.errors.push(...validation.errors);
                result.errorRows++;
                continue;
        ...
        try {
            ...
            await queryRunner.commitTransaction();
            await this.auditService.logBulkImport(organisationId, 'contract_config', result.importId, actorUserId, `Imported ${result.createdCount} new, updated ${result.updatedCount} existing contract configurations`, { totalRows: result.totalRows, validRows: result.validRows, errorRows: result.errorRows });
        }
        catch (error) {
            await queryRunner.rollbackTransaction();
            throw error;
    ...
    async importHolidays(rows, isDryRun, actorUserId, result) {
        const validatedRows = [];
        for (let i = 0; i < rows.length; i++) {
            const row = rows[i];
            const rowNum = i + 2;
        ...
        try {
            for (const { data } of validatedRows) {
                ...
                await queryRunner.manager.insert(HolidayEntity, {
                    ...
                    date: holidayDate,
                    name: data.name,
                    holidayType: data.type ?? HolidayType.PUBLIC,
                    isRecurring,
            ...
            await queryRunner.commitTransaction();
            await this.auditService.logBulkImport('system', 'holidays', result.importId, actorUserId, `Imported ${result.createdCount} holidays`, { totalRows: result.totalRows, validRows: result.validRows, errorRows: result.errorRows });
        }
        catch (error) {
            await queryRunner.rollbackTransaction();
            throw error;
    ...
    validateConfigRow(row, rowNum, requiredIdField) {
        const errors = [];
...
CsvImportService = CsvImportService_1 = __decorate([
    Injectable(),
    __param(0, InjectRepository(CompanyDebitConfigurationEntity)),
    __param(1, InjectRepository(ClientDebitConfigurationEntity)),
    __param(2, InjectRepository(ContractDebitConfigurationEntity)),
    __param(3, InjectRepository(HolidayEntity)),
