...
export class CatalogueOutgoingWebhookService {
   ...
   *
   * @param keycloakGroupId - Organisation UUID
   * @returns Result object with success status, count, and optional error
   */
  async pushAllProducts(keycloakGroupId: string): Promise<{
    success: boolean;
    ...
    try {
      ...
      this.logger.log(`Loaded ${allProducts.length} active products for organisation ${keycloakGroupId}`);

      // 2. Filter out products where sourceDerniereModif === 'webhook_partner' (anti-loop)
      const productsToSync = allProducts.filter((p) => p.sourceDerniereModif !== 'webhook_partner');
      ...
      const payload = productsToSync.map((p) => ({
        id: p.id,
        keycloakGroupId: p.keycloakGroupId,
        gammeId: p.gammeId,
        sku: p.sku,
        nom: p.nom,
