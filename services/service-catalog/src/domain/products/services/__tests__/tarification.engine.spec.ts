...
function createEngineFixture(
  input: { produit?: TestProduit; formules?: TestFormule[]; prixGrilles?: TestPrixGrille[] } = {},
): TarificationService {
  ...
  const produitRepository = {
    findOne: async ({ where }: { where: { id: string } }) => (where.id === produit.id ? produit : null),
  ...
  const formuleProduitRepository = {
    findOne: async ({ where }: { where: { id?: string; produitId: string; code?: string; actif: boolean } }) =>
