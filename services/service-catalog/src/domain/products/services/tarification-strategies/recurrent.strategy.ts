...
export class RecurrentTarificationStrategy implements ITarificationStrategy {
  ...
  calculate(produit: ProduitEntity, quantite: number, _options?: TarificationCalculationOptions): PrixCalcule {
    ...
    const prixMensuel = Number.isNaN(config.prixMensuel) ? Number(produit.prix || 0) : config.prixMensuel;

    const prixUnitaire =
      frequence === 'ANNUEL' || frequence === 'ANNUAL' ? this.round(prixMensuel * 12) : this.round(prixMensuel);
