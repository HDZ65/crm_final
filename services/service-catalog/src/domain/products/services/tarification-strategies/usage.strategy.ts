...
import type { ITarificationStrategy, PrixCalcule, TarificationCalculationOptions } from '../tarification.engine';
...
export class UsageTarificationStrategy implements ITarificationStrategy {
  ...
  calculate(produit: ProduitEntity, quantite: number, _options?: TarificationCalculationOptions): PrixCalcule {
    const quantiteNormalisee = Math.max(1, Math.floor(Number(quantite || 0)));
    const config = this.parseConfig(produit.configTarification);
    const prixParUnite = Number.isNaN(config.prixParUnite) ? Number(produit.prix || 0) : config.prixParUnite;

    const brut = this.round(prixParUnite * quantiteNormalisee);
    const avecPlancher = Math.max(brut, this.round(config.plancherMensuel));
    const borne =
      config.plafondMensuel !== null ? Math.min(avecPlancher, this.round(config.plafondMensuel)) : avecPlancher;
  ...
  private parseConfig(config: Record<string, unknown> | null): UsageConfig {
    ...
    const plafondMensuel =
      config?.plafondMensuel === null || config?.plafondMensuel === undefined ? null : Number(config.plafondMensuel);
