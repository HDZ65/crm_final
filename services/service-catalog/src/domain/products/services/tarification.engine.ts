...
export interface ITarificationStrategy {
  ...
  calculate(produit: ProduitEntity, quantite: number, options?: TarificationCalculationOptions): PrixCalcule;
}

export interface ITarificationEngine {
  calculate(produitId: string, quantite: number, options?: TarificationCalculationOptions): Promise<PrixCalcule>;
}
...
export class TarificationService implements ITarificationEngine {
  ...
  private async resolvePrixGrille(produitId: string, grilleTarifaireId?: string): Promise<PrixProduitEntity | null> {
