...
function createFixture(subscriptions: SubscriptionEntity[] = []) {
  let lastDueForChargeQuery: { keycloakGroupId: string; beforeDate: Date } | null = null;
  let lastDueForTrialQuery: string | null = null;

  const repository = {
    getDueForCharge: async (keycloakGroupId: string, beforeDate: Date) => {
      lastDueForChargeQuery = { keycloakGroupId, beforeDate };
      return subscriptions;
    },
    getDueForTrialConversion: async (keycloakGroupId: string) => {
      lastDueForTrialQuery = keycloakGroupId;
      return subscriptions;
    },
...
describe('SubscriptionSchedulingService', () => {
  it('calculateNextChargeAt adds one month for MONTHLY frequency', () => {
    ...
    const next = service.calculateNextChargeAt(SubscriptionFrequency.MONTHLY, new Date('2026-01-31T00:00:00.000Z'));
  ...
  it('calculateNextChargeAt adds one year for ANNUAL frequency', () => {
    ...
    const next = service.calculateNextChargeAt(SubscriptionFrequency.ANNUAL, new Date('2026-01-15T00:00:00.000Z'));
  ...
  it('getDueForCharge delegates to repository query', async () => {
    ...
    const result = await service.getDueForCharge('org-1', new Date('2026-02-15T00:00:00.000Z'));
  ...
  it('getDueForTrialConversion filters expired trial subscriptions', async () => {
    ...
    const result = await service.getDueForTrialConversion('org-1', new Date('2026-02-10T00:00:00.000Z'));
