...
import { SubscriptionStateMachineService, SubscriptionStatus } from '../subscription-state-machine.service';
...
function makeSubscription(status: SubscriptionStatus, overrides: Partial<SubscriptionEntity> = {}): SubscriptionEntity {
  return {
    id: 'sub-1',
...
describe('SubscriptionStateMachineService', () => {
  ...
  describe('transition', () => {
    it('creates history entry with expected fields', () => {
      ...
      const result = service.transition(subscription, SubscriptionStatus.ACTIVE, 'manual activation', 'user-123');
