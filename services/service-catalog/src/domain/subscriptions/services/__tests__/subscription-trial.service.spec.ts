...
function createFixture(options: FixtureOptions = {}) {
  ...
  let lastSchedulingQuery: { keycloakGroupId: string; now: Date | undefined } | null = null;

  const schedulingService = {
    getDueForTrialConversion: async (keycloakGroupId: string, now?: Date) => {
      lastSchedulingQuery = { keycloakGroupId, now };
  ...
  const service = new SubscriptionTrialService(schedulingService, chargeService, lifecycleService, natsService as any, {
