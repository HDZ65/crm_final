...
export class PreferenceValidationService {
  validateAgainstSchema(values: Record<string, any>, schemas: SubscriptionPreferenceSchemaEntity[]): ValidationResult {
    const errors: string[] = [];
    ...
    for (const [code, value] of Object.entries(values)) {
      ...
      if (!this.validateType(value, schema.valueType)) {
        errors.push(`Preference '${code}' has invalid type (expected ${schema.valueType})`);
        continue;
      }
      ...
      if (schema.valueType === PreferenceValueType.ENUM && schema.allowedValues) {
        if (!schema.allowedValues.includes(String(value))) {
          errors.push(`Preference '${code}' must be one of: ${schema.allowedValues.join(', ')}`);
        }
      }
    }
