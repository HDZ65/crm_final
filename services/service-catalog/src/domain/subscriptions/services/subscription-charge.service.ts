...
export interface SubscriptionChargeSchedulingPort {
  getDueSubscriptions(keycloakGroupId: string, beforeDate: Date): Promise<SubscriptionEntity[]>;
...
export class PaymentServiceGrpcClient implements SubscriptionPaymentClient {
  ...
  async createPaymentIntent(input: CreateSubscriptionPaymentIntentInput): Promise<SubscriptionPaymentIntent> {
    const request: CreatePaymentIntentGrpcRequest = {
      organisation_id: input.keycloakGroupId,
...
export class FactureServiceGrpcClient implements SubscriptionFactureClient {
  ...
  constructor(options: FactureServiceGrpcClientOptions = {}) {
    ...
    this.statutId = options.statutId || process.env.SUBSCRIPTION_FACTURE_STATUT_ID || DEFAULT_FACTURE_STATUT_ID;
    this.emissionFactureId =
      options.emissionFactureId || process.env.SUBSCRIPTION_FACTURE_EMISSION_ID || DEFAULT_FACTURE_EMISSION_ID;
    this.adresseFacturationId =
      options.adresseFacturationId || process.env.SUBSCRIPTION_FACTURE_ADRESSE_ID || DEFAULT_FACTURE_ADRESSE_ID;
    this.defaultProduitId =
      options.defaultProduitId || process.env.SUBSCRIPTION_FACTURE_PRODUIT_ID || DEFAULT_FACTURE_PRODUIT_ID;

    const parsedTauxTva = Number(
      options.defaultTauxTva ?? process.env.SUBSCRIPTION_FACTURE_TAUX_TVA ?? DEFAULT_FACTURE_TAUX_TVA,
    );
    this.defaultTauxTva = Number.isNaN(parsedTauxTva) ? DEFAULT_FACTURE_TAUX_TVA : parsedTauxTva;
  }
...
export class SubscriptionChargeService {
  ...
  constructor(
    ...
    private readonly idempotencyStore: IdempotenceStore = new InMemorySubscriptionChargeIdempotencyStore(),
  ...
  async processCharges(keycloakGroupId: string): Promise<ChargeResult> {
    const now = this.now();
    const dueSubscriptions = await this.schedulingService.getDueSubscriptions(keycloakGroupId, now);
    const results: SubscriptionChargeResult[] = [];

    for (const subscription of dueSubscriptions) {
      if (subscription.keycloakGroupId !== keycloakGroupId) {
        results.push(
          this.buildSkippedResult(subscription.id, this.safeIdempotencyKey(subscription), 'ORGANISATION_MISMATCH'),
        );
        continue;
      }
  ...
  private async processSubscriptionCharge(
    ...
    if (!this.isWebDirectSubscription(subscription)) {
      return this.buildSkippedResult(subscription.id, this.safeIdempotencyKey(subscription), 'STORE_SOURCE_EXCLUDED');
    }

    if (subscription.planType === SubscriptionPlanType.FREE_AVOD) {
      return this.buildSkippedResult(subscription.id, this.safeIdempotencyKey(subscription), 'FREE_PLAN_NO_CHARGE');
    }

    const status = subscription.status as SubscriptionStatus;
    if (!options.allowedStatuses.includes(status)) {
      return this.buildSkippedResult(subscription.id, this.safeIdempotencyKey(subscription), 'STATUS_NOT_ELIGIBLE');
    }

    if (!subscription.nextChargeAt) {
      return this.buildSkippedResult(subscription.id, this.safeIdempotencyKey(subscription), 'NEXT_CHARGE_AT_MISSING');
    }
    ...
    try {
      ...
      const facture = await this.factureClient.createFacture({
        keycloakGroupId: savedSubscription.keycloakGroupId,
      ...
      await this.idempotenceService.markProcessed(idempotencyKey, SUBSCRIPTION_CHARGE_IDEMPOTENCE_EVENT);
      ...
      await this.publishEvent(SUBSCRIPTION_CHARGED_EVENT, {
        ...
        idempotencyKey,
        chargedAt,
        nextChargeAt: savedSubscription.nextChargeAt ? this.toIsoDate(savedSubscription.nextChargeAt) : null,
  ...
  private buildPaymentIntentInput(
    ...
    return {
      keycloakGroupId: subscription.keycloakGroupId,
  ...
  private buildSkippedResult(subscriptionId: string, idempotencyKey: string, reason: string): SubscriptionChargeResult {
    return {
      subscriptionId,
      idempotencyKey,
      status: 'SKIPPED',
  ...
  private normalizeStoreSource(value: StoreSource | string | null | undefined): StoreSource {
    const normalized = String(value || StoreSource.NONE)
      .trim()
      .toUpperCase();
  ...
  private shouldTransitionToPastDue(policy: ChargeFailureTransitionPolicy, retryCount: number): boolean {
    if (policy === 'ALWAYS') {
      return true;
    }
  ...
  private resolveSocieteId(subscription: SubscriptionEntity): string {
    ...
    return subscription.keycloakGroupId;
  }
