...
import { type SubscriptionEntity, SubscriptionPlanType, SubscriptionStatus } from '../entities/subscription.entity';
...
export class SubscriptionLifecycleService {
  constructor(
    ...
    private readonly statusHistoryRepository: ISubscriptionHistoryRepository | ISubscriptionStatusHistoryRepository,
    ...
    return this.transition({
      ...
      onBeforeSave: (subscription) => {
        if (this.schedulingService && subscription.nextChargeAt) {
          ...
          subscription.nextChargeAt = nextChargeAt instanceof Date ? nextChargeAt : new Date(nextChargeAt);
        }
  ...
  async cancel(subscriptionId: string, reason: string, cancelAtPeriodEnd: boolean): Promise<SubscriptionEntity>;
  ...
  private async requireSubscription(subscriptionId: string): Promise<SubscriptionEntity> {
    ...
    if (!subscription) {
      throw new DomainException(`Subscription ${subscriptionId} not found`, 'SUBSCRIPTION_NOT_FOUND', {
        subscriptionId,
      });
    }
    return subscription;
