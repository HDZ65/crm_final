...
import { type SubscriptionChargeResult, SubscriptionChargeService } from './subscription-charge.service';
...
export interface SubscriptionTrialSchedulingPort {
  getDueForTrialConversion(keycloakGroupId: string, now?: Date): Promise<SubscriptionEntity[]>;
}
...
export class SubscriptionTrialService {
  ...
  async processTrialConversions(keycloakGroupId: string): Promise<TrialConversionResult> {
    const now = this.now();
    const dueTrials = await this.schedulingService.getDueForTrialConversion(keycloakGroupId, now);
    const results: TrialConversionItemResult[] = [];

    for (const subscription of dueTrials) {
      try {
        const result = await this.processSingleTrialConversion(keycloakGroupId, subscription);
        results.push(result);
      } catch (error) {
        results.push({
