...
export class SubscriptionPreferenceController {
  ...
  async upsertSchema(data: {
    ...
    const schema = await this.schemaService.upsert({
      keycloakGroupId: data.organisation_id,
      code: data.code,
      label: data.label,
  ...
  async setPreference(data: {
    ...
    for (const pref of data.preferences) {
      ...
      const existing = await this.preferenceService.findBySubscriptionAndSchema(data.subscription_id, schema.id);
  ...
  private mapSchemaToResponse(schema: any) {
    return {
      id: schema.id,
      organisation_id: schema.keycloakGroupId,
      code: schema.code,
      label: schema.label,
  ...
  private mapPreferenceToResponse(pref: any) {
    return {
      id: pref.id,
      organisation_id: pref.keycloakGroupId,
