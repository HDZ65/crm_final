...
export class SubscriptionController {
  ...
  async getDueForCharge(data: GetDueForChargeRequest) {
    const beforeDate = data.beforeDate ? new Date(data.beforeDate) : new Date();
    const subscriptions = await this.subscriptionService.getDueForCharge(data.organisationId, beforeDate);
    return {
      subscriptions,
      pagination: {
  ...
  async getDueForTrialConversion(data: GetDueForTrialConversionRequest) {
    const subscriptions = await this.subscriptionService.getDueForTrialConversion(data.organisationId);
    return {
      subscriptions,
      pagination: {
        total: subscriptions.length,
