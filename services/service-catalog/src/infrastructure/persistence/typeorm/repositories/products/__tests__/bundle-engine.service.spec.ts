...
import { BundleEngineService, BundleServiceCode } from '../bundle-engine.service';
...
function createFixture(configuration = makeConfiguration()) {
  ...
  const configurationRepository = {
    findOne: async ({ where }: { where?: { id?: string; keycloakGroupId?: string; actif?: boolean } } = {}) => {
      ...
      if (where?.keycloakGroupId && where.keycloakGroupId !== configuration.keycloakGroupId) {
        return null;
      }
