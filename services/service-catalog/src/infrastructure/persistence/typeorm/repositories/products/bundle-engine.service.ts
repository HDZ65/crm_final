...
export class BundleEngineService {
  ...
  async calculatePrice(clientId: string, services: string[], keycloakGroupId?: string): Promise<BundlePriceResult> {
    const configuration = await this.getConfiguration(keycloakGroupId);
  ...
  async recalculateOnServiceChange(input: RecalculateOnServiceChangeInput): Promise<BundlePriceResult> {
    const configuration = await this.getConfiguration(input.keycloakGroupId);
  ...
  async getConfigurationByOrganisation(keycloakGroupId: string): Promise<ConfigurationBundleEntity | null> {
    return this.configurationBundleRepository.findOne({ where: { keycloakGroupId, actif: true } });
  }
  ...
  private calculateFromConfiguration(
    ...
    if (hasConciergerie && hasJustiPlus && hasWincash) {
      ...
      if (hasConciergerie && hasWincash) {
        discountByService[BundleServiceCode.WINCASH] = this.round(
          Math.min(basePriceMap[BundleServiceCode.WINCASH], Number(configuration.remiseWincashAvecConciergerie || 0)),
        );
      }
    }
    ...
    return {
      clientId,
      keycloakGroupId: configuration.keycloakGroupId,
  ...
  private async getConfiguration(keycloakGroupId?: string): Promise<ConfigurationBundleEntity> {
    const configuration = keycloakGroupId
      ? await this.configurationBundleRepository.findOne({ where: { keycloakGroupId, actif: true } })
