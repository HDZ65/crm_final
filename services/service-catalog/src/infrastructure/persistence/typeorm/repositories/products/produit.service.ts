...
import {
  ...
  StatutCycleProduit,
  TypeProduit,
} from '../../../../../domain/products/entities/produit.entity';
...
export class ProduitService {
  ...
  async create(input: {
    ...
    const existing = await this.produitRepository.findOne({
      where: { keycloakGroupId: input.organisation_id, sku: input.sku },
    });
  ...
  async update(input: {
    ...
    if (input.categorie !== undefined)
      produit.categorie = categorieFromProto[input.categorie as ProtoCategorieProduitType];
    if (input.type !== undefined) produit.type = typeFromProto[input.type as ProtoTypeProduitType];
    if (input.statut_cycle !== undefined)
      produit.statutCycle = statutCycleFromProto[input.statut_cycle as ProtoStatutCycleProduitType];
  ...
  async findBySku(keycloakGroupId: string, sku: string): Promise<ProduitEntity> {
    const produit = await this.produitRepository.findOne({
      where: { keycloakGroupId, sku },
      relations: ['gamme'],
    });
  ...
  async findAll(input: {
    ...
    const queryBuilder = this.produitRepository
      ...
      .where('produit.keycloak_group_id = :keycloakGroupId', {
        keycloakGroupId: input.organisation_id,
      });
    ...
    if (search) {
      queryBuilder
        .addSelect("ts_rank(produit.search_vector, plainto_tsquery('french', :search))", 'rank')
        .andWhere("produit.search_vector @@ plainto_tsquery('french', :search)", { search })
        .orderBy('rank', 'DESC');
    } else {
      queryBuilder.orderBy(`produit.${sortBy}`, sortOrder);
    }
