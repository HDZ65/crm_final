...
export class SubscriptionPlanService implements ISubscriptionPlanRepository {
  ...
  async findByCode(keycloakGroupId: string, code: SubscriptionPlanType): Promise<SubscriptionPlanEntity | null> {
  ...
  async update(input: {
    ...
    if (input.amount !== undefined && input.priceAnnual === undefined && input.priceMonthly === undefined) {
  ...
  private normalizeCode(code?: string): SubscriptionPlanType {
    const normalized = String(code || SubscriptionPlanType.FREE_AVOD)
      .trim()
      .toUpperCase();
