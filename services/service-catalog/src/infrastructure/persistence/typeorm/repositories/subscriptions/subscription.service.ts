...
export class SubscriptionService implements ISubscriptionRepository {
  ...
  async getDueForTrialConversion(keycloakGroupId: string): Promise<SubscriptionEntity[]> {
    return this.repository.find({
      where: {
        keycloakGroupId,
  ...
  async findDueForCharge(keycloakGroupId: string, beforeDate: string): Promise<SubscriptionEntity[]> {
    return this.getDueForCharge(keycloakGroupId, new Date(beforeDate));
  }
  ...
  private normalizeStoreSource(value?: string): StoreSource {
    const normalized = String(value || StoreSource.WEB_DIRECT)
      .trim()
      .toUpperCase();
