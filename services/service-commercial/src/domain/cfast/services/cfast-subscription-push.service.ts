...
export class CfastSubscriptionPushService {
  ...
  async assignSubscription(
    ...
    if (!contrat) {
      throw new Error(`Contrat ${contratId} not found for organisation ${organisationId}`);
    }

    // 3. Ensure client is pushed to CFAST and get SITE mapping
    const cfastSiteId = await this.ensureClientSite(organisationId, contrat.clientId);
    ...
    const serviceData: CfastCreateServiceDto = {
      ...
      price: contrat.montant != null ? Number(contrat.montant) : undefined,
      billingFrequency: contrat.frequenceFacturation || undefined,
    };
    ...
    this.logger.log(
      `Created CFAST service ${cfastServiceId} for contrat ${contratId} under site ${cfastSiteId}`,
    );
   ...
   */
  private async ensureClientSite(organisationId: string, clientId: string): Promise<string> {
    // Check for existing CLIENT→SITE mapping
    let siteMapping = await this.cfastEntityMappingService.findMapping(
      organisationId,
      'CLIENT',
      clientId,
      'SITE',
    ...
    this.logger.log(
      `No CLIENT→SITE mapping for client ${clientId}, pushing client to CFAST first...`,
    );
    await this.cfastClientPushService.pushClient(organisationId, clientId);

    // Re-fetch the mapping after push
    siteMapping = await this.cfastEntityMappingService.findMapping(
      organisationId,
      'CLIENT',
      clientId,
      'SITE',
    ...
    if (!siteMapping) {
      throw new Error(
        `Failed to obtain CFAST site mapping for client ${clientId} after push`,
      );
    }
