...
} from '../../entities/reducbox-access.entity';
...
function createService(mocks: ReturnType<typeof createMocks>) {
  return new ReducBoxLifecycleService(
    mocks.reducBoxPort,
    mocks.reducBoxAccessRepository,
    mocks.natsService,
  );
}
...
describe('ReducBoxLifecycleService', () => {
  ...
  describe('createAccess()', () => {
    ...
    it('should propagate error when port fails', async () => {
      (mocks.reducBoxPort.createAccess as ReturnType<typeof jest.fn>).mockRejectedValue(
        new Error('EXTERNAL_API_DOWN'),
      );

      await expect(service.createAccess('client-001', 'contrat-001')).rejects.toThrow(
        'EXTERNAL_API_DOWN',
      );
    });
  });
  ...
  describe('suspendAccess()', () => {
    ...
    it('should throw when access has no externalAccessId', async () => {
      ...
      await expect(service.suspendAccess('access-001', 'reason')).rejects.toThrow(
        'no external access ID',
      );
    });
  });
  ...
  describe('restoreAccess()', () => {
    ...
    it('should throw when access not found', async () => {
      ...
      await expect(service.restoreAccess('unknown-id')).rejects.toThrow(
        'ReducBox access not found: unknown-id',
      );
    });
    ...
    it('should throw when access has no externalAccessId', async () => {
      ...
      await expect(service.restoreAccess('access-001')).rejects.toThrow(
        'no external access ID',
      );
    });
  });
