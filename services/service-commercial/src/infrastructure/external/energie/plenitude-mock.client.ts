...
@Injectable()
export class PlenitudeMockClient implements EnergiePartenairePort {
  private readonly logger = new Logger(PlenitudeMockClient.name);
  ...
  async createRaccordement(data: {
    ...
    const raccordementId = `plenitude-${data.clientId}`;
  ...
  private recordCall(method: string, payload: Record<string, unknown>): void {
    ...
    this.logger.log(
      `[MOCK PLENITUDE] ${method} called at ${calledAt} with payload ${JSON.stringify(payload)}`,
    );
  }
}
