...
import { CQRejectedHandler } from '../cq-rejected.handler';
...
function createHandler(mocks: ReturnType<typeof createMocks>) {
  return new CQRejectedHandler(mocks.natsService, mocks.contratRepository, mocks.cqRepository);
}
...
describe('CQRejectedHandler', () => {
  ...
  it('subscribes to contrat.cq.rejected on init', async () => {
    ...
    expect(subscribeArgs[0]).toBe('contrat.cq.rejected');
  });

  it('updates contrat.statutCq to REJETE', async () => {
    const { subscriptions } = await initAndGetSubscriptions(mocks);

    await subscriptions['contrat.cq.rejected']({
    ...
    expect(updateArgs[1]).toEqual({ statutCq: StatutCQ.REJETE });
  });

  it('updates ControleQualite record with REJETE status and motif', async () => {
    const { subscriptions } = await initAndGetSubscriptions(mocks);

    await subscriptions['contrat.cq.rejected']({
      contrat_id: 'contrat-1',
      organisation_id: 'org-1',
      motif: 'Signature manquante',
      timestamp: '2026-03-16T10:00:00.000Z',
    });
    ...
    expect(savedEntity.motifRejet).toBe('Signature manquante');
  ...
  it('handles empty motif (null fallback)', async () => {
    const { subscriptions } = await initAndGetSubscriptions(mocks);
    ...
    expect(mocks.cqRepository.save).toHaveBeenCalledTimes(1);
    const savedEntity = (mocks.cqRepository.save as ReturnType<typeof jest.fn>).mock.calls[0][0];
    expect(savedEntity.motifRejet).toBeNull(); // '' || null → null
  });
});
