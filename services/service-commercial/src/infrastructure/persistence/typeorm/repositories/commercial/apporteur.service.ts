...
import { ApporteurEntity } from '../../../../../domain/commercial/entities/apporteur.entity';

@Injectable()
export class ApporteurService {
  constructor(
    @InjectRepository(ApporteurEntity)
    private readonly apporteurRepository: Repository<ApporteurEntity>,
  ) {}

  async create(data: Partial<ApporteurEntity>): Promise<ApporteurEntity> {
    if (data.organisationId && data.nom && data.prenom) {
      ...
      if (existing) {
        throw new RpcException({
          ...
          message: `Un commercial "${data.prenom} ${data.nom}" existe deja`,
        });
      }
    }
    ...
    if (data.organisationId && data.email) {
      ...
      if (existingEmail) {
        throw new RpcException({
          code: status.ALREADY_EXISTS,
          message: `Un commercial avec l'email "${data.email}" existe deja`,
        });
      }
    }
  ...
  async update(id: string, data: Partial<ApporteurEntity>): Promise<ApporteurEntity> {
    const apporteur = await this.findById(id);
    Object.assign(apporteur, data);
    return this.apporteurRepository.save(apporteur);
  }

  async findById(id: string): Promise<ApporteurEntity> {
    const apporteur = await this.apporteurRepository.findOne({ where: { id } });
    if (!apporteur) {
      throw new NotFoundException(`Apporteur ${id} non trouve`);
    }
    return apporteur;
  }

  async findByUtilisateur(utilisateurId: string): Promise<ApporteurEntity> {
    const apporteur = await this.apporteurRepository.findOne({ where: { utilisateurId } });
    if (!apporteur) {
      throw new NotFoundException(`Apporteur pour utilisateur ${utilisateurId} non trouve`);
    }
    return apporteur;
  }
  ...
  async activer(id: string): Promise<ApporteurEntity> {
    const apporteur = await this.findById(id);
    apporteur.actif = true;
    return this.apporteurRepository.save(apporteur);
  }

  async desactiver(id: string): Promise<ApporteurEntity> {
    const apporteur = await this.findById(id);
