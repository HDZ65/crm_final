...
export class ControleQualiteService {
  ...
  async update(
    id: string,
    updates: Partial<ControleQualiteEntity>,
  ): Promise<ControleQualiteEntity> {
    try {
    ...
    try {
      ...
      if (this.natsService && this.natsService.isConnected()) {
        await this.natsService.publish('contrat.cq.validated', {
          ...
          organisation_id: controle.organisationId,
        ...
        this.logger.warn(`NATS not available — skipping contrat.cq.validated publish for contrat ${controle.contratId}`);
      }
