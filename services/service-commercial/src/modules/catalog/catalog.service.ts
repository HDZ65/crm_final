...
export class CatalogService {
  ...
  async getCatalog(input: GetCatalogRequest): Promise<GetCatalogResult> {
    const { organisation_id, grille_tarifaire_id, gamme_id, include_inactive } = input;
    this.logger.log(`Getting catalog for organisation ${organisation_id}`);

    // Get the pricing grid
    let grille: GrilleTarifaireEntity;
    if (grille_tarifaire_id) {
      grille = await this.grilleService.findById({ id: grille_tarifaire_id });
    } else {
    ...
    const produits = await this.produitService.findByOrganisation({
      organisation_id,
      actif: include_inactive ? undefined : true,
    ...
    for (const produit of produits) {
      ...
      if (gamme_id && produit.gammeId !== gamme_id) {
        continue;
      }
  ...
  async calculatePrice(input: CalculatePriceRequest): Promise<CalculatePriceResult> {
    const { produit_id, grille_tarifaire_id, quantite, remise_additionnelle = 0 } = input;
    const produit = await this.produitService.findById({ id: produit_id });
    const now = new Date();
    const enPromotion = this.isPromotionActive(produit, now);
    ...
    // Calculate discount
    let remiseTotale = remise_additionnelle;
    if (prixProduit) {
