...
export class OrchestrationController {
  ...
  async activateContract(data: OrchestrationRequest) {
    ...
    const history = await this.orchestrationService.activate(data.contract_id, payload);
    return {
  ...
  async suspendContract(data: OrchestrationRequest) {
    const payload = data.payload ? JSON.parse(data.payload) : undefined;
    const history = await this.orchestrationService.suspend(data.contract_id, payload);
    return {
      success: history.status === 'success',
  ...
  async terminateContract(data: OrchestrationRequest) {
    const payload = data.payload ? JSON.parse(data.payload) : undefined;
    const history = await this.orchestrationService.terminate(data.contract_id, payload);
    return {
      success: history.status === 'success',
  ...
  async portInContract(data: OrchestrationRequest) {
    const payload = data.payload ? JSON.parse(data.payload) : undefined;
    const history = await this.orchestrationService.portIn(data.contract_id, payload);
    return {
      success: history.status === 'success',
  ...
  async getOrchestrationHistory(data: GetOrchestrationHistoryRequest) {
    const result = await this.orchestrationService.getHistory(data.contract_id, data.pagination);
    return {
