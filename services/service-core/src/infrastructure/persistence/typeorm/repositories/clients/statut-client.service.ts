...
import { StatutClientEntity } from '../../../../../domain/clients/entities';

@Injectable()
export class StatutClientService {
  private readonly logger = new Logger(StatutClientService.name);

  constructor(
    @InjectRepository(StatutClientEntity)
    private readonly repository: Repository<StatutClientEntity>,
  ) {}
  ...
  async create(input: {
    ...
    if (existing) {
      throw new RpcException({ code: status.ALREADY_EXISTS, message: `Statut with code ${input.code} already exists` });
    }
  ...
  async findById(id: string): Promise<StatutClientEntity> {
    const entity = await this.repository.findOne({ where: { id } });
    if (!entity) {
      throw new RpcException({ code: status.NOT_FOUND, message: `Statut client ${id} not found` });
    }
    return entity;
  }

  async findByCode(code: string): Promise<StatutClientEntity> {
    const entity = await this.repository.findOne({ where: { code } });
    if (!entity) {
      throw new RpcException({ code: status.NOT_FOUND, message: `Statut client with code ${code} not found` });
    }
    return entity;
  }

  async findAll(pagination?: { page?: number; limit?: number }): Promise<{
    statuts: StatutClientEntity[];
