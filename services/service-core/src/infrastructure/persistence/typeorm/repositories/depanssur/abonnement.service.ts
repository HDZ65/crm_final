...
export class AbonnementService implements IAbonnementRepository {
  ...
  async create(input: any): Promise<AbonnementDepanssurEntity> {
    const entity = this.repository.create({
      keycloakGroupId: input.keycloakGroupId || input.organisation_id,
      ...
      plafondParIntervention:
        (input.plafondParIntervention ?? input.plafond_par_intervention != null)
          ? String(input.plafondParIntervention ?? input.plafond_par_intervention)
          : null,
      plafondAnnuel:
        (input.plafondAnnuel ?? input.plafond_annuel != null)
      ...
      montantRemise:
        (input.montantRemise ?? input.montant_remise != null)
  ...
  async update(input: any): Promise<AbonnementDepanssurEntity> {
    ...
    if (input.periodeAttente ?? input.periode_attente !== undefined)
      entity.periodeAttente = input.periodeAttente ?? input.periode_attente;
    ...
    if (input.montantHt ?? input.montant_ht !== undefined)
      entity.montantHt = String(input.montantHt ?? input.montant_ht);
    if (input.codeRemise ?? input.code_remise !== undefined)
      entity.codeRemise = input.codeRemise ?? input.code_remise ?? null;
    if (input.montantRemise ?? input.montant_remise !== undefined) {
      entity.montantRemise =
        (input.montantRemise ?? input.montant_remise) != null
          ? String(input.montantRemise ?? input.montant_remise)
          : null;
    }
    ...
    if (entity.statut !== oldStatut) {
      ...
      await this.publishAbonnementStatusChangedEvent(
        saved,
        oldStatut,
        entity.statut,
        input.motifResiliation ?? input.motif_resiliation,
      );
    }
  ...
  async findByClientId(keycloakGroupId: string, clientId: string): Promise<AbonnementDepanssurEntity | null> {
    return this.repository.findOne({
  ...
  async findAll(
    ...
    const qb = this.repository
      ...
      .where('a.keycloakGroupId = :orgId', { orgId: keycloakGroupId });
