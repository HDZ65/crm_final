...
export class TacheService {
  ...
  async create(data: Partial<TacheEntity>): Promise<TacheEntity> {
    const tache = this.tacheRepository.create(data);
    return this.tacheRepository.save(tache);
  }

  async update(id: string, data: Partial<TacheEntity>): Promise<TacheEntity> {
    const tache = await this.findById(id);
    Object.assign(tache, data);
    return this.tacheRepository.save(tache);
  }

  async findById(id: string): Promise<TacheEntity> {
  ...
  async getAlertes(organisationId: string): Promise<{
    enRetard: TacheEntity[];
    echeanceDemain: TacheEntity[];
  }> {
    const now = new Date();
