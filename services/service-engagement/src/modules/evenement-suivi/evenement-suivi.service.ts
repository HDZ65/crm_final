...
export class EvenementSuiviService {
  constructor(
    @InjectRepository(EvenementSuivi)
    private readonly evenementSuiviRepository: Repository<EvenementSuivi>,
  ) {}

  async create(data: Partial<EvenementSuivi>): Promise<EvenementSuivi> {
    const evenement = this.evenementSuiviRepository.create(data);
    return this.evenementSuiviRepository.save(evenement);
  }

  async update(id: string, data: Partial<EvenementSuivi>): Promise<EvenementSuivi> {
    const evenement = await this.findById(id);
    Object.assign(evenement, data);
    return this.evenementSuiviRepository.save(evenement);
  }

  async findById(id: string): Promise<EvenementSuivi> {
