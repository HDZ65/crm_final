...
@Injectable()
export class EmailInvoiceCreatedHandler implements OnModuleInit {
  private readonly logger = new Logger(EmailInvoiceCreatedHandler.name);
  ...
  private async handleInvoiceCreated(event: InvoiceCreatedEvent): Promise<void> {
    ...
    try {
      ...
      this.logger.log(
        `Invoice PDF email queued for invoice ${invoiceId} (client: ${clientId}, amount: ${montant}EUR)`,
      );

      await this.processedEventsRepo.markProcessed(eventId, 'email.invoice.created');
    } catch (error) {
