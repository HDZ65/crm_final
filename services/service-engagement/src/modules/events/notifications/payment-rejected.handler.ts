...
@Injectable()
export class NotificationPaymentRejectedHandler {
  private readonly logger = new Logger(NotificationPaymentRejectedHandler.name);
  ...
  async handle(@Payload() event: PaymentRejectedEvent, @Ctx() context: NatsContext) {
    ...
    this.logger.log(`Payment rejected: ${paymentId}, creating high-priority alert`);
    
    await this.processedEventsRepo.markProcessed(eventId, 'notification.payment.rejected');
  }
}
