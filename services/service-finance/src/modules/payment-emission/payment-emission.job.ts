...
export class PaymentEmissionJob {
  ...
  async publishPaymentRejectedEvent(payment: PaymentIntentEntity): Promise<boolean> {
    ...
    const event: PaymentRejectedEvent = {
      ...
      client_id: clientId,
      motif_rejet: this.extractRejectionCode(payment),
      date_rejet: { seconds: Math.floor(Date.now() / 1000), nanos: 0 },
    };
    ...
    try {
      ...
      this.logger.log(
        `Published PaymentRejectedEvent: ${eventId} for payment ${payment.id} (motif: ${event.motif_rejet})`,
      );
      return true;
    } catch (error: unknown) {
