...
export type ListSessionsParams = Pick<ListPortalSessionsRequest, 'organisation_id' | 'customer_id'> & {
  contract_id?: string;
...
export type GetAuditParams = Pick<GetPortalSessionAuditRequest, 'session_id'> & {
  page?: number;
  limit?: number;
};
...
export class PortalQueryService {
  ...
  async listSessions(params: ListSessionsParams): Promise<ListSessionsResult> {
    ...
    if (params.contract_id) {
      where.contractId = params.contract_id;
    }
  ...
  async getSessionAudit(params: GetAuditParams): Promise<GetAuditResult> {
    ...
    const [events, total] = await this.auditRepository.findAndCount({
      where: { portalSessionId: params.session_id },
      order: { createdAt: 'ASC' },
      skip,
      take: limit,
    });
