...
export class PortalSessionService {
  ...
  async createSession(params: CreateSessionParams): Promise<CreateSessionResult> {
    const idempotencyKeyHash = params.idempotency_key
      ? this.hashString(params.idempotency_key)
      : null;
    ...
    const ttlSeconds = params.ttl_seconds ?? DEFAULT_TTL_SECONDS;
    const expiresAt = new Date(Date.now() + ttlSeconds * 1000);

    const session = this.sessionRepository.create({
      organisationId: params.organisation_id,
      societeId: params.societe_id,
      ...
      expiresAt,
      maxUses: params.max_uses ?? DEFAULT_MAX_USES,
      useCount: 0,
      idempotencyKeyHash,
      amountCents: params.amount_cents,
      currency: params.currency ?? 'EUR',
      description: params.description ?? null,
      mandateId: params.mandate_id ?? null,
      rumMasked: params.rum_masked ?? null,
    ...
    await this.createAuditEntry({
      ...
      data: {
        ...
        maxUses: (params.max_uses ?? DEFAULT_MAX_USES).toString(),
        amountCents: params.amount_cents.toString(),
        currency: params.currency ?? 'EUR',
      },
    });

    this.logger.log(`Created portal session ${savedSession.id} for customer ${params.customer_id}`);
