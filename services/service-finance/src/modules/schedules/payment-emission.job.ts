      ...
      if (organisationId) {
        try {
          await this.retryClient.handlePaymentRejected({
            ...
            facture_id: schedule.factureId ?? undefined,
            contrat_id: schedule.contratId ?? undefined,
            client_id: schedule.clientId,
            reason_code: 'EMISSION_FAILED',
            reason_message: error.message,
            amount_cents: Math.round(paymentIntent.amount * 100),
            currency: paymentIntent.currency,
            psp_name: schedule.provider.toUpperCase(),
            psp_payment_id: paymentIntent.providerPaymentId ?? undefined,
            rejected_at: { seconds: Math.floor(Date.now() / 1000), nanos: 0 },
            event_timestamp: { seconds: Math.floor(Date.now() / 1000), nanos: 0 },
            idempotency_key: `${paymentIntent.id}:emission_failed`,
          });
  ...
  private async calculateNextPlannedDate(schedule: ScheduleEntity): Promise<Date> {
    ...
    try {
      const response = await this.calendarClient.calculatePlannedDate({
        ...
        client_id: schedule.clientId ?? '',
        societe_id: schedule.societeId ?? '',
        reference_date: current.toISOString().slice(0, 10),
        target_month: next.getMonth() + 1,
        target_year: next.getFullYear(),
        include_resolution_trace: false,
      });

      return new Date(response.planned_debit_date);
    } catch (error: any) {
