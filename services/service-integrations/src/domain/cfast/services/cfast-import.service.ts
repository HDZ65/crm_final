...
export class CfastImportService {
  ...
  async importInvoices(keycloakGroupId: string): Promise<ImportResult> {
    ...
    try {
      ...
      const existingExternalIds = await this.loadExistingExternalIds(keycloakGroupId);
      const billingSessions = await this.cfastApiClient.listBillingSessions(token);

      if (billingSessions.length === 0) {
        await this.importInvoicesForSession(token, undefined, keycloakGroupId, existingExternalIds, result);
      } else {
        for (const billingSession of billingSessions) {
          await this.importInvoicesForSession(token, billingSession.id, keycloakGroupId, existingExternalIds, result);
        }
      }
  ...
  async getImportStatus(keycloakGroupId: string): Promise<ImportStatusResult> {
    const config = await this.cfastConfigService.findByOrganisationId(keycloakGroupId);
    if (!config) {
      throw new Error(`CFAST config not found for organisation ${keycloakGroupId}`);
    }
  ...
  private async importInvoicesForSession(
    ...
    for (const invoice of invoices) {
      try {
        await this.importSingleInvoice(invoice, keycloakGroupId, existingExternalIds);
  ...
  private async importSingleInvoice(
    ...
    const matchedClient = await this.findMatchingClient(keycloakGroupId, customerNom, customerPrenom, customerPhone);
  ...
  private async loadExistingExternalIds(keycloakGroupId: string): Promise<Set<string>> {
