...
import { BaseCalcul, TypeCalcul } from '../../../commercial/entities/bareme-commission.entity';
...
describe('CommissionChannelService', () => {
  ...
  it('getApplicableBareme: FREE_AVOD returns null (no commission)', async () => {
    const result = await service.getApplicableBareme(SubscriptionPlanType.FREE_AVOD, 'web_direct', 'org-123');
  ...
  it('getApplicableBareme: PREMIUM_SVOD with web_direct returns barème', async () => {
    ...
    const result = await service.getApplicableBareme(SubscriptionPlanType.PREMIUM_SVOD, 'web_direct', 'org-123');
  ...
  it('getApplicableBareme: VIP with apple_store returns lower rate barème', async () => {
    ...
    const result = await service.getApplicableBareme(SubscriptionPlanType.VIP, 'apple_store', 'org-123');
  ...
  it('calculateCommission: PREMIUM_SVOD web_direct calculates correct commission', async () => {
    ...
    // 9.99 * 15% = 1.4985 ≈ 1.50
    expect(commission).toBeCloseTo(1.5, 2);
  });
