...
export class StoreBillingService {
  ...
  constructor(private readonly storeBillingRepository: StoreBillingRecordService) {}
   ...
   */
  async recordStorePayment(subscriptionId: string, storeEvent: StoreEventInput): Promise<StoreBillingRecordEntity> {
    ...
    record.keycloakGroupId = storeEvent.keycloakGroupId;
   ...
   *
   * @param keycloakGroupId - Organisation ID
   * @param period - Optional date range filter
   * @returns Array of revenue stats per store source
   */
  async getRevenueByStore(keycloakGroupId: string, _period?: StoreBillingPeriod): Promise<StoreRevenueStats[]> {
    if (!keycloakGroupId) {
      throw new DomainException('Organisation ID is required', 'STORE_BILLING_MISSING_ORGANISATION_ID');
    }

    const storeSources = [StoreSource.APPLE_STORE, StoreSource.GOOGLE_STORE, StoreSource.TV_STORE];

    const results: StoreRevenueStats[] = [];

    for (const storeSource of storeSources) {
      const aggregate = await this.storeBillingRepository.aggregateRevenueByStore(keycloakGroupId, storeSource);
    ...
    this.logger.log(
      `Revenue aggregated for organisation ${keycloakGroupId}: ${results.length} store sources with transactions`,
    );
   ...
   */
  async getSubscriptionStoreHistory(subscriptionId: string): Promise<StoreBillingRecordEntity[]> {
    if (!subscriptionId) {
      throw new DomainException('Subscription ID is required', 'STORE_BILLING_MISSING_SUBSCRIPTION_ID');
    }

    // Repository already orders by eventDate DESC
    const records = await this.storeBillingRepository.findBySubscription(
      '', // keycloakGroupId not required for this query
      subscriptionId,
    );

    this.logger.log(`Retrieved ${records.length} store billing records for subscription ${subscriptionId}`);
  ...
  private determineStatusFromEventType(eventType: StoreEventType): StoreBillingStatus {
    switch (eventType) {
      ...
      default:
        throw new DomainException(`Unknown store event type: ${eventType}`, 'STORE_BILLING_UNKNOWN_EVENT_TYPE', {
          eventType,
        });
    }
  }
