...
export class WinLeadPlusMapperService {
  mapProspectToClient(prospect: WinLeadPlusProspect): WinLeadPlusMappedClient {
    ...
    const payment = Array.isArray(prospect.informationsPaiement) ? prospect.informationsPaiement[0] : undefined;
    ...
    const ibanVal = iban || undefined;
    const bicVal = bic || undefined;
    const civiliteVal = this.toText(prospect.civilite) || undefined;
    const cspVal = this.toText(prospect.csp) || undefined;
    const regimeSocialVal = this.toText(prospect.regimeSocial) || undefined;
    const lieuNaissanceVal = this.toText(prospect.lieuNaissance) || undefined;
    const paysNaissanceVal = this.toText(prospect.paysNaissance) || undefined;
    const etapeCouranteVal = this.toText(prospect.etapeCourante) || undefined;
    const numssVal = this.toText(prospect.numss) || undefined;
    const mandatSepaVal = typeof mandatSepaValue === 'boolean' ? mandatSepaValue : undefined;
    const isPoliticallyExposedVal =
      typeof prospect.is_politically_exposed === 'boolean' ? prospect.is_politically_exposed : undefined;
    const adresseVal =
    ...
    return {
      ...
      ...(ibanVal !== undefined ? { iban: ibanVal } : {}),
      ...(mandatSepaVal !== undefined ? { mandat_sepa: mandatSepaVal } : {}),
      ...(civiliteVal !== undefined ? { civilite: civiliteVal } : {}),
      ...(bicVal !== undefined ? { bic: bicVal } : {}),
      ...(cspVal !== undefined ? { csp: cspVal } : {}),
      ...(regimeSocialVal !== undefined ? { regime_social: regimeSocialVal } : {}),
      ...(lieuNaissanceVal !== undefined ? { lieu_naissance: lieuNaissanceVal } : {}),
      ...(paysNaissanceVal !== undefined ? { pays_naissance: paysNaissanceVal } : {}),
      ...(etapeCouranteVal !== undefined ? { etape_courante: etapeCouranteVal } : {}),
      ...(isPoliticallyExposedVal !== undefined ? { is_politically_exposed: isPoliticallyExposedVal } : {}),
      ...(numssVal !== undefined ? { numss: numssVal } : {}),
      ...(adresseVal !== undefined ? { adresse: adresseVal } : {}),
    };
  }
  ...
  mapContratToContrat(contrat: WinLeadPlusContrat, clientId: string): WinLeadPlusMappedContrat {
    ...
    const dateSignature1 = this.toText(contrat.dateSignature || contrat.date_signature) || undefined;
    const frequenceFacturation1 = this.toText(contrat.frequenceFacturation || contrat.frequence_facturation) || undefined;
    const notes1 = this.toText(contrat.notes) || undefined;
    const montant1 = this.toNumber(contrat.montant);
    ...
    return {
      ...
      ...(montant1 !== null ? { montant: montant1 } : {}),
      ...(dateSignature1 !== undefined ? { dateSignature: dateSignature1 } : {}),
      ...
      ...(frequenceFacturation1 !== undefined ? { frequenceFacturation: frequenceFacturation1 } : {}),
      fournisseur: WINLEADPLUS_SOURCE,
      clientId,
      commercialId: this.toText(contrat.commercialId),
      ...(notes1 !== undefined ? { notes: notes1 } : {}),
      source: WINLEADPLUS_SOURCE,
    };
  }

  mapAbonnementToLigneContrat(abonnement: WinLeadPlusAbonnement, contratId: string): WinLeadPlusMappedLigneContrat {
  ...
  mapSouscriptionContratToContrat(
    ...
    const dateSignature2 = this.toText(contrat.dateSignature || contrat.date_signature) || undefined;
    const frequenceFacturation2 = this.toText(contrat.frequenceFacturation || contrat.frequence_facturation) || undefined;
    const notes2 = this.toText(contrat.notes) || undefined;
    const montant2 = this.toNumber(souscription.totalAmount ?? contrat.montant);

    return {
      reference: `WLP-CONTRAT-${externalId}`,
      titre: this.toText(contrat.titre) || `Contrat ${externalId}`,
      statut: this.toText(contrat.statut) || 'ACTIF',
      ...(montant2 !== null ? { montant: montant2 } : {}),
      ...(dateSignature2 !== undefined ? { dateSignature: dateSignature2 } : {}),
      dateDebut: this.toText(contrat.dateSignature || contrat.date_signature) || new Date().toISOString(),
      devise: this.toText(contrat.devise) || 'EUR',
      ...(frequenceFacturation2 !== undefined ? { frequenceFacturation: frequenceFacturation2 } : {}),
      fournisseur,
      clientId,
      commercialId: commercialId || this.toText(contrat.commercialId) || '',
      ...(notes2 !== undefined ? { notes: notes2 } : {}),
      source: WINLEADPLUS_SOURCE,
    };
  }
