...
import { type SaveWinLeadPlusConfigInput, WinLeadPlusSyncService } from './services/winleadplus-sync.service';
...
export class WinLeadPlusGrpcController {
  ...
  async syncProspects(data: SyncProspectsRequest, metadata?: RpcMetadata) {
    ...
    const syncLog = await this.syncService.syncProspects(data.organisation_id, Boolean(data.dry_run), { token });
  ...
  async saveConfig(data: UpdateWinLeadPlusConfigRequest) {
    ...
    const input: SaveWinLeadPlusConfigInput = {
      ...
      ...(data.organisation_id !== undefined ? { keycloakGroupId: data.organisation_id } : {}),
      ...(data.api_endpoint !== undefined ? { apiEndpoint: data.api_endpoint } : {}),
      ...(data.enabled !== undefined ? { enabled: data.enabled } : {}),
      ...(data.sync_interval_minutes !== undefined ? { syncIntervalMinutes: data.sync_interval_minutes } : {}),
      ...(data.api_token !== undefined ? { apiToken: data.api_token } : {}),
    };
  ...
  private mapSyncLog(log: {
    ...
    return {
      id: log.id,
      organisation_id: log.keycloakGroupId,
  ...
  private mapConfig(config: {
    ...
    return {
      id: config.id,
      organisation_id: config.keycloakGroupId,
