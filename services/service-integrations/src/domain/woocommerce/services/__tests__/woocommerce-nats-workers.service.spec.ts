...
function createWorkerFixture() {
  ...
  const syncService = {
    syncCustomer: async (...args: any[]) => {
      syncCalls.push({ method: 'syncCustomer', args });
      return { clientId: 'c-1', isNew: true };
    },
    syncSubscriptionCreated: async (...args: any[]) => {
      syncCalls.push({ method: 'syncSubscriptionCreated', args });
      return { subscriptionId: 's-1' };
    },
    syncSubscriptionUpdated: async (...args: any[]) => {
      syncCalls.push({ method: 'syncSubscriptionUpdated', args });
      return { subscriptionId: 's-1', updated: true };
    },
    syncOrderCompleted: async (...args: any[]) => {
      syncCalls.push({ method: 'syncOrderCompleted', args });
      return { orderId: 'o-1', linkedSubscriptionId: null };
    },
    syncPaymentSucceeded: async (...args: any[]) => {
      syncCalls.push({ method: 'syncPaymentSucceeded', args });
      return { paymentId: 'p-1', linkedSubscriptionId: null };
    },
  } as unknown as WooCommerceSyncService;

  const webhookService = {
    markProcessing: async (eventId: string) => {
      markCalls.push({ method: 'markProcessing', eventId });
    },
    markProcessed: async (eventId: string) => {
      markCalls.push({ method: 'markProcessed', eventId });
    },
    markFailed: async (eventId: string, error: string) => {
      markCalls.push({ method: 'markFailed', eventId, error });
    },
...
describe('WooCommerceNatsWorkersService', () => {
  ...
  it('worker marks event as FAILED when sync throws', async () => {
    ...
    const webhookSvc = {
      markProcessing: async (eid: string) => {
        markCalls.push({ method: 'markProcessing', eventId: eid });
      },
      markProcessed: async (eid: string) => {
        markCalls.push({ method: 'markProcessed', eventId: eid });
      },
      markFailed: async (eid: string, err: string) => {
        markCalls.push({ method: 'markFailed', eventId: eid, error: err });
      },
    } as unknown as WooCommerceWebhookService;
