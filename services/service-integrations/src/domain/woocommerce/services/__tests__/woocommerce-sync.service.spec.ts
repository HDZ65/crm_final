...
import { type ClientBasePort, type ClientSearchResult, WooCommerceSyncService } from '../woocommerce-sync.service';
...
function createFixture(
  ...
  for (const m of options.existingMappings || []) {
    mappings.set(`${m.keycloakGroupId}:${m.entityType}:${m.wooId}`, m);
  }
  ...
  const mappingRepository: IWooCommerceMappingRepository = {
    ...
    findByCrmEntityId: async (crmId) => {
      const r: WooCommerceMappingEntity[] = [];
      for (const m of mappings.values()) {
        if (m.crmEntityId === crmId) r.push(m);
      }
      return r;
    },
    ...
    save: async (entity) => {
      if (!entity.id) {
        mappingIdCounter++;
        entity.id = `mapping-${mappingIdCounter}`;
      }
      mappings.set(`${entity.keycloakGroupId}:${entity.entityType}:${entity.wooId}`, entity);
  ...
  const subscriptionRepository = {
    ...
    save: async (entity: SubscriptionEntity) => {
      if (!entity.id) {
        subIdCounter++;
        (entity as any).id = `sub-${subIdCounter}`;
      }
  ...
  const clientPort: ClientBasePort = {
    ...
    createClient: async (input) => {
      const client = {
        id: `client-new-${clientCreated.length + 1}`,
        email: input.email,
    ...
    updateClient: async (clientId, input) => {
      clientUpdated.push({ clientId, input });
      return { id: clientId, ...input } as ClientSearchResult;
    },
  };
...
describe('WooCommerceSyncService', () => {
  ...
  it('syncCustomer updates existing mapping when already synced', async () => {
    ...
    existingMapping.keycloakGroupId = 'org-1';
