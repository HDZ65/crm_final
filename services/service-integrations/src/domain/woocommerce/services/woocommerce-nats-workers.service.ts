...
export class WooCommerceNatsWorkersService implements OnModuleInit {
  ...
  private async registerWorkers(): Promise<void> {
    ...
    await this.natsService.subscribe<WooCommerceNatsMessage>('woocommerce.customer.created', async (data) =>
      this.handleCustomerCreated(data),
    );

    // Worker 2: Subscription Created
    await this.natsService.subscribe<WooCommerceNatsMessage>('woocommerce.subscription.created', async (data) =>
      this.handleSubscriptionCreated(data),
    );

    // Worker 3: Subscription Updated
    await this.natsService.subscribe<WooCommerceNatsMessage>('woocommerce.subscription.updated', async (data) =>
      this.handleSubscriptionUpdated(data),
    );

    // Worker 4: Order Completed
    await this.natsService.subscribe<WooCommerceNatsMessage>('woocommerce.order.completed', async (data) =>
      this.handleOrderCompleted(data),
    );

    // Worker 5: Payment Intent Succeeded
    await this.natsService.subscribe<WooCommerceNatsMessage>('woocommerce.payment_intent.succeeded', async (data) =>
  ...
  async handleCustomerCreated(data: WooCommerceNatsMessage): Promise<void> {
    ...
    try {
      ...
      const result = await this.syncService.syncCustomer(data.keycloakGroupId, data.wooResourceId, data.payload);

      this.logger.log(`Customer synced: woo=${data.wooResourceId} → crm=${result.clientId} (new=${result.isNew})`);

      await this.webhookService.markProcessed(data.eventId);
    } catch (error) {
      this.logger.error(`Failed to process customer.created ${data.wooResourceId}: ${error}`);
      await this.webhookService.markFailed(data.eventId, error instanceof Error ? error.message : String(error));
    }
  }
  ...
  async handleSubscriptionCreated(data: WooCommerceNatsMessage): Promise<void> {
    ...
    try {
      ...
      const result = await this.syncService.syncSubscriptionCreated(
        data.keycloakGroupId,
        data.wooResourceId,
        data.payload,
      );

      this.logger.log(`Subscription created: woo=${data.wooResourceId} → crm=${result.subscriptionId}`);

      await this.webhookService.markProcessed(data.eventId);
    } catch (error) {
      this.logger.error(`Failed to process subscription.created ${data.wooResourceId}: ${error}`);
      await this.webhookService.markFailed(data.eventId, error instanceof Error ? error.message : String(error));
    }
  }
  ...
  async handleSubscriptionUpdated(data: WooCommerceNatsMessage): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`Failed to process subscription.updated ${data.wooResourceId}: ${error}`);
      await this.webhookService.markFailed(data.eventId, error instanceof Error ? error.message : String(error));
    }
  }
  ...
  async handleOrderCompleted(data: WooCommerceNatsMessage): Promise<void> {
    ...
    try {
      ...
      const result = await this.syncService.syncOrderCompleted(data.keycloakGroupId, data.wooResourceId, data.payload);
      ...
      this.logger.error(`Failed to process order.completed ${data.wooResourceId}: ${error}`);
      await this.webhookService.markFailed(data.eventId, error instanceof Error ? error.message : String(error));
    }
  }
  ...
  async handlePaymentSucceeded(data: WooCommerceNatsMessage): Promise<void> {
    ...
    try {
      ...
      this.logger.log(`Payment synced: woo=${data.wooResourceId} → crm=${result.paymentId}`);

      await this.webhookService.markProcessed(data.eventId);
    } catch (error) {
      this.logger.error(`Failed to process payment_intent.succeeded ${data.wooResourceId}: ${error}`);
      await this.webhookService.markFailed(data.eventId, error instanceof Error ? error.message : String(error));
    }
  }
}
