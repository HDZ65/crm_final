...
import { WooCommerceMappingService } from '../../../infrastructure/persistence/typeorm/repositories/woocommerce/woocommerce-mapping.service';
...
export interface ClientBasePort {
  searchByEmail(keycloakGroupId: string, email: string): Promise<ClientSearchResult | null>;
  searchByPhone(keycloakGroupId: string, phone: string): Promise<ClientSearchResult | null>;
...
export class WooCommerceSyncService {
  ...
  constructor(
    private readonly mappingRepository: WooCommerceMappingService,
    private readonly subscriptionRepository: SubscriptionService,
  ...
  async syncCustomer(
    ...
    if (this.clientPort) {
      if (email) {
        existingClient = await this.clientPort.searchByEmail(keycloakGroupId, email);
      }
      if (!existingClient && phone) {
        existingClient = await this.clientPort.searchByPhone(keycloakGroupId, phone);
      }
    }
    ...
    if (existingClient) {
      ...
      this.logger.log(`Reconciled WooCommerce customer ${wooCustomerId} with existing CRM client ${clientId}`);
    } else {
      // Create new client
      if (this.clientPort) {
  ...
  async syncSubscriptionUpdated(
    ...
    if (!existingMapping) {
      ...
      const result = await this.syncSubscriptionCreated(keycloakGroupId, wooSubscriptionId, payload);
      return { subscriptionId: result.subscriptionId, updated: false };
    }

    // Update existing subscription
    const subscription = await this.subscriptionRepository.findById(existingMapping.crmEntityId);
  ...
  async syncOrderCompleted(
    ...
    if (subscriptionIds.length > 0 && subscriptionIds[0] !== undefined) {
