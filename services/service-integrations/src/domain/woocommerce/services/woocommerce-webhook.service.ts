...
import { WooCommerceConfigService } from '../../../infrastructure/persistence/typeorm/repositories/woocommerce/woocommerce-config.service';
import { WooCommerceWebhookEventService } from '../../../infrastructure/persistence/typeorm/repositories/woocommerce/woocommerce-webhook-event.service';
...
export class WooCommerceWebhookService {
  ...
  constructor(
    private readonly webhookEventRepository: WooCommerceWebhookEventService,
    private readonly configRepository: WooCommerceConfigService,
  ...
  async processWebhook(request: WebhookRequest): Promise<WebhookResult> {
    ...
    const config = await this.configRepository.findByOrganisationId(request.keycloakGroupId);
    ...
    if (request.deliveryId) {
      const existing = await this.webhookEventRepository.findByExternalEventId('woocommerce', request.deliveryId);
      if (existing) {
    ...
    const isValid = this.validateSignature(request.body, request.signature, config.webhookSecret);

    if (!isValid) {
      this.logger.warn(`Invalid webhook signature for organisation ${request.keycloakGroupId}`);
  ...
  async retryEvent(eventId: string): Promise<WebhookResult> {
    ...
    if (natsSubject) {
      await this.publishToNats(natsSubject, {
        eventId: event.id,
        keycloakGroupId: event.keycloakGroupId,
