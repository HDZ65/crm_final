...
export class MockImsClient implements IImsClient {
  ...
  private recordCall<T>(method: string, payload: Record<string, any>, data: T): ImsClientCallResult<T> {
    const calledAt = new Date().toISOString();
    this.calls.push({ method, payload, calledAt });

    this.logger.log(`[MOCK IMS] ${method} called at ${calledAt} with payload ${JSON.stringify(payload)}`);
