...
function createFixture(options?: { natsConnected?: boolean }) {
  ...
  const imsWebhookEventService = {
    ...
    create: async (input: {
      ...
      entity.keycloakGroupId = input.keycloakGroupId;
  ...
  const controller = new ImsWebhookController(imsWebhookEventService as any, natsService as any);
...
describe('ImsWebhookController', () => {
  ...
  it('accepts a valid IMS webhook, stores it, and marks it as RECEIVED', async () => {
    ...
    const response = await controller.handleWebhook(makeRequest(rawBody) as any, body, signature);
  ...
  it('publishes valid webhook events to NATS', async () => {
    ...
    expect(publishedMessages[0].subject).toBe('crm.commercial.mondial-tv.ims.webhook.received');
  ...
  it('rejects webhook requests without signature header', async () => {
    ...
    await expect(controller.handleWebhook(makeRequest(rawBody) as any, body, '')).rejects.toBeInstanceOf(
  ...
  it('rejects webhook requests when event_id is missing', async () => {
    ...
    await expect(controller.handleWebhook(makeRequest(rawBody) as any, body as any, signature)).rejects.toBeInstanceOf(
      BadRequestException,
    );
  });
  ...
  it('uses payload.organisation_id when missing at root level', async () => {
    ...
    expect(storedEvents[0].keycloakGroupId).toBe('org-from-payload');
  });
  ...
  it('fails with 500 when IMS secret is not configured', async () => {
    ...
    await expect(controller.handleWebhook(makeRequest(rawBody) as any, body, signature)).rejects.toBeInstanceOf(
