...
import { IdempotenceService, type IdempotenceStore, NatsService } from '@crm/shared-kernel';
import {
  BadRequestException,
  Body,
  Controller,
...
export class ImsWebhookController {
  ...
  constructor(
    ...
    this.idempotenceService = new IdempotenceService(new ImsWebhookIdempotenceStore(this.imsWebhookEventService));
  }
  ...
  async handleWebhook(
    ...
    const keycloakGroupId = this.resolveOrganisationId(body);
    ...
    if (!secret) {
      throw new InternalServerErrorException('IMS webhook secret is not configured (IMS_WEBHOOK_SECRET)');
    }
  ...
  private resolveOrganisationId(body: ImsWebhookPayload): string | null {
    const topLevelOrganisationId = typeof body?.organisation_id === 'string' ? body.organisation_id.trim() : '';
    if (topLevelOrganisationId) {
      return topLevelOrganisationId;
    }

    const nestedOrganisationId =
      typeof body?.payload?.organisation_id === 'string' ? body.payload.organisation_id.trim() : '';
  ...
  private validateSignature(rawBody: string, signature: string, secret: string): boolean {
    ...
    return (
      this.secureCompare(normalizedSignature, expectedHex) || this.secureCompare(normalizedSignature, expectedBase64)
    );
  }
  ...
  private getParisOffsetMillis(date: Date): number {
    ...
    const parts = formatter.formatToParts(date).reduce<Record<string, string>>((carry, part) => {
