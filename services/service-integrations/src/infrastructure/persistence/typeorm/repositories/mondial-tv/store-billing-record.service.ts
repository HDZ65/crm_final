...
export class StoreBillingRecordService implements IStoreBillingRecordRepository {
  ...
  async findBySubscription(keycloakGroupId: string, subscriptionId: string): Promise<StoreBillingRecordEntity[]> {
    return this.repository.find({
      where: { keycloakGroupId, subscriptionId },
      order: { eventDate: 'DESC' },
    });
  }

  async findByStoreTransaction(
  ...
  async findByStatus(keycloakGroupId: string, status: StoreBillingStatus): Promise<StoreBillingRecordEntity[]> {
    return this.repository.find({
      where: { keycloakGroupId, status },
      order: { createdAt: 'DESC' },
    });
  }

  async aggregateRevenueByStore(
    ...
    const result = await this.repository
      ...
      .where('sbr.organisation_id = :keycloakGroupId', { keycloakGroupId })
