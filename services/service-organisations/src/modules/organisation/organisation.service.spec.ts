...
describe('OrganisationService', () => {
  ...
  describe('create', () => {
    ...
    it('should use provided ID if specified', async () => {
      ...
      const result = await service.create(inputWithId);
  ...
  describe('update', () => {
    ...
    it('should update organisation successfully', async () => {
      const updatedOrg = { ...mockOrganisation, nom: 'Organisation Mise à Jour', email: 'nouveau@email.com' };
      repository.findOne.mockResolvedValue({ ...mockOrganisation });
      repository.save.mockResolvedValue(updatedOrg);

      const result = await service.update(updateInput);

      expect(result.nom).toBe('Organisation Mise à Jour');
    ...
    it('should only update provided fields', async () => {
      repository.findOne.mockResolvedValue({ ...mockOrganisation });
      repository.save.mockImplementation((entity) => Promise.resolve(entity as OrganisationEntity));

      await service.update({ id: 'org-uuid-1', nom: 'Nouveau Nom' });

      expect(repository.save).toHaveBeenCalledWith(
        expect.objectContaining({
          nom: 'Nouveau Nom',
          email: 'contact@test-org.com', // unchanged
        }),
      );
    });

    it('should allow updating actif status', async () => {
      repository.findOne.mockResolvedValue({ ...mockOrganisation });
      repository.save.mockImplementation((entity) => Promise.resolve(entity as OrganisationEntity));

      await service.update({ id: 'org-uuid-1', actif: false });
    ...
    it('should allow updating etat', async () => {
      repository.findOne.mockResolvedValue({ ...mockOrganisation });
      repository.save.mockImplementation((entity) => Promise.resolve(entity as OrganisationEntity));

      await service.update({ id: 'org-uuid-1', etat: 'suspendu' });
  ...
  describe('findAll', () => {
    ...
    it('should order by nom ASC', async () => {
      await service.findAll();

      expect(mockQueryBuilder.orderBy).toHaveBeenCalledWith('o.nom', 'ASC');
    });
  });
