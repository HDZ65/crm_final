...
export class RetrySchedulerService {
  ...
  async handlePaymentRejected(event: PaymentRejectedEvent): Promise<{
    ...
    this.logger.log(`Handling payment rejection: ${event.paymentId} with code ${event.reasonCode}`);
    ...
    if (existing) {
      this.logger.warn(`Duplicate rejection event detected: ${event.idempotencyKey}`);
      return {
        processed: false,
    ...
    if (!policy) {
      this.logger.warn(`No retry policy found for org ${event.organisationId}`);
      return {
        processed: false,
    ...
    const eligibilityResult = this.determineEligibility(event.reasonCode, policy);
    const nextRetryDate = eligibilityResult.eligibility === RetryEligibility.ELIGIBLE
      ? this.calculateNextRetryDate(event.rejectedAt, policy.retryDelaysDays, 0)
      : null;
    ...
    const schedule = this.scheduleRepository.create({
      ...
      factureId: event.factureId || null,
      contratId: event.contratId || null,
      clientId: event.clientId,
      rejectionCode: this.normalizeRejectionCode(event.reasonCode),
      rejectionRawCode: event.reasonCode,
      rejectionMessage: event.reasonMessage,
