...
function createService(mocks: ReturnType<typeof createMocks>) {
  return new ControleQualiteService(mocks.cqRepository, mocks.statutRepository, mocks.natsService);
}
...
describe('ControleQualiteService', () => {
  ...
  describe('create (soumettreControle)', () => {
    it('creates a controle with EN_ATTENTE status by default', async () => {
      ...
      expect(createArg.keycloakGroupId).toBe('org-1');
    ...
    it('throws RpcException on repository error', async () => {
      (mocks.cqRepository.save as ReturnType<typeof jest.fn>).mockRejectedValue(new Error('DB_FAILURE'));
  ...
  describe('findById', () => {
    ...
    it('throws RpcException on DB error', async () => {
      (mocks.cqRepository.findOne as ReturnType<typeof jest.fn>).mockRejectedValue(new Error('DB_ERROR'));
  ...
  describe('findByStatut', () => {
    it('returns paginated results', async () => {
      ...
      expect(findArg.where.keycloakGroupId).toBe('org-1');
  ...
  describe('full workflow: create → valider', () => {
    it('EN_ATTENTE → VALIDE with NATS event', async () => {
      ...
      expect(mocks.natsService.publish).toHaveBeenCalledTimes(1);
      expect((mocks.natsService.publish as ReturnType<typeof jest.fn>).mock.calls[0][0]).toBe('contrat.cq.validated');
    });
  });
  ...
  describe('full workflow: create → rejeter', () => {
    it('EN_ATTENTE → REJETE with motif + NATS event', async () => {
      ...
      expect(mocks.natsService.publish).toHaveBeenCalledTimes(1);
      expect((mocks.natsService.publish as ReturnType<typeof jest.fn>).mock.calls[0][0]).toBe('contrat.cq.rejected');
    });
  });
});
