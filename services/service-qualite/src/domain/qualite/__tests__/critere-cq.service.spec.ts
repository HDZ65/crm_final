...
function createRepository() {
  ...
  return {
    find: jest.fn(async (opts?: { where?: Partial<CritereCQEntity>; order?: Record<string, string> }) => {
      if (opts?.where?.keycloakGroupId) {
        return store.filter((c) => c.keycloakGroupId === opts.where?.keycloakGroupId);
      }
      return [...store];
    }),
    findOne: jest.fn(async (opts?: { where?: Partial<CritereCQEntity> }) => {
      ...
      if (opts?.where?.code && opts?.where?.keycloakGroupId) {
        return (
          store.find((c) => c.code === opts.where?.code && c.keycloakGroupId === opts.where?.keycloakGroupId) ?? null
        );
      }
      return null;
    }),
...
describe('CritereCQ Repository CRUD', () => {
  ...
  describe('create', () => {
    it('creates a critere with required fields', async () => {
      ...
      expect(saved.keycloakGroupId).toBe('org-1');
    });
  ...
  describe('findOne', () => {
    ...
    it('finds by code + keycloakGroupId (unique constraint)', async () => {
      const result = await repo.findOne({
        where: { code: 'DOC_CNI', keycloakGroupId: 'org-1' },
      });

      expect(result).toBeDefined();
      expect(result?.code).toBe('DOC_CNI');
      expect(result?.keycloakGroupId).toBe('org-1');
    });
  });
  ...
  describe('findByOrganisation', () => {
    it('returns all criteres for a given organisation', async () => {
      const result = await repo.find({ where: { keycloakGroupId: 'org-1' } });

      expect(repo.find).toHaveBeenCalledTimes(1);
      expect(result).toHaveLength(2);
      expect(result.every((c) => c.keycloakGroupId === 'org-1')).toBe(true);
    });

    it('returns empty array for organisation with no criteres', async () => {
      const result = await repo.find({ where: { keycloakGroupId: 'org-unknown' } });

      expect(result).toHaveLength(0);
    });

    it('org-2 has only 1 critere', async () => {
      const result = await repo.find({ where: { keycloakGroupId: 'org-2' } });
