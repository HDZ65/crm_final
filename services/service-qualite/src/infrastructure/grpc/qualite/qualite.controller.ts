...
function mapControleToResponse(entity: any) {
  return {
    id: entity.id,
    organisation_id: entity.keycloakGroupId,
...
export class QualiteController {
  ...
  constructor(private readonly controleQualiteService: ControleQualiteService) {}
  ...
  async creerControle(data: CreerControleRequest) {
    try {
      const controle = await this.controleQualiteService.create(data.organisation_id, data.contrat_id);
      return mapControleToResponse(controle);
    } catch (error) {
