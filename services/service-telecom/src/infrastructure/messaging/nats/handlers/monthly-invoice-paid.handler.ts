...
@Injectable()
export class MonthlyInvoicePaidHandler implements OnModuleInit {
  constructor(
  ...
  async onModuleInit(): Promise<void> {
    await this.natsService.subscribe<IncomingInvoicePayload>(
      'crm.telecom.facture_mensuelle_payee',
  ...
  async handle(payload: IncomingInvoicePayload): Promise<void> {
    ...
    await this.sagaService.registerMonthlyInvoicePaid(event);
  }
}
