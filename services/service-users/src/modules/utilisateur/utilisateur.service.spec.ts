...
describe('UtilisateurService', () => {
  ...
  describe('create', () => {
    ...
    it('should throw ALREADY_EXISTS if email exists', async () => {
      repository.findOne.mockResolvedValue(mockUtilisateur);
    ...
    it('should set actif to true by default', async () => {
      ...
      await service.create({ ...createInput, actif: undefined });
    ...
    it('should handle null telephone', async () => {
      ...
      await service.create({ ...createInput, telephone: undefined });
  ...
  describe('findByKeycloakId', () => {
    it('should return utilisateur when found', async () => {
      ...
      expect(result).toEqual(mockUtilisateur);
      expect(repository.findOne).toHaveBeenCalledWith({ where: { keycloakId: 'keycloak-uuid-1' } });
    });
  ...
  describe('findByEmail', () => {
    it('should return utilisateur when found', async () => {
      ...
      const result = await service.findByEmail('jean.dupont@example.com');
  ...
  describe('update', () => {
    ...
    it('should only update provided fields', async () => {
      repository.findOne.mockResolvedValue({ ...mockUtilisateur });
      repository.save.mockImplementation((entity) => Promise.resolve(entity as UtilisateurEntity));

      await service.update({ id: 'user-uuid-1', nom: 'Nouveau' });
    ...
    it('should allow deactivating user', async () => {
      ...
      await service.update({ id: 'user-uuid-1', actif: false });
  ...
  describe('findAll', () => {
    ...
    it('should order by nom and prenom', async () => {
      await service.findAll();

      expect(mockQueryBuilder.orderBy).toHaveBeenCalledWith('u.nom', 'ASC');
      expect(mockQueryBuilder.addOrderBy).toHaveBeenCalledWith('u.prenom', 'ASC');
    });
